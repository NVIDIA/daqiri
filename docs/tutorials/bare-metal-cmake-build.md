...
!!! tip "DGX Spark"
    ...
    On DGX Spark, generate the concrete config from the Spark ports, cores, and RX MAC. See [Generate a raw-Ethernet pair](../config-generation.md#generate-a-raw-ethernet-pair).
...
=== "DGX Spark (GB10)"
    ...
    - For a runnable end-to-end test after the build completes, follow the [DGX Spark profile callout](../benchmarks/raw_benchmarking.md#update-the-loopback-configuration) and generate the selected raw or RoCE pair from the discovered system parameters.
