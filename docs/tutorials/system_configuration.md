...
DAQIRI requires a system with an [**NVIDIA SmartNIC**](https://www.nvidia.com/en-us/networking/ethernet-adapters/) (ConnectX-6 Dx or later) and a [**discrete GPU**](https://www.nvidia.com/en-us/design-visualization/desktop-graphics/) (GPUDirect-capable). Check the [Getting Started](../getting-started.md) page to confirm you are running on a supported platform.

This page covers both the **required system setup** to get DAQIRI running and **optional performance tuning** to maximize throughput and minimize latency. Complete the [System Setup for DAQIRI](#system-setup-for-daqiri) section first, then move on to [System Optimization](#system-optimization) as needed.
