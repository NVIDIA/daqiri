...
??? question "1. I want to measure baseline throughput"
    ...
    **Raw Ethernet** (`stream_type: "raw"`) — runs on `daqiri_bench_raw_gpudirect`.
    ...
    - **DGX Spark multi-queue core-scaling matrix** (prefilled) — four configs derived from the four-queue base that vary the TX/RX queue+core counts to show how throughput scales (TX → cores 16,17; RX → cores 18,19), e.g. the (TX=1, RX=1) baseline [`daqiri_bench_raw_tx_rx_spark_mq_1t1r.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_raw_tx_rx_spark_mq_1t1r.yaml). The other cells are [`_1t2r`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_raw_tx_rx_spark_mq_1t2r.yaml) (RX scaling), [`_2t1r`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_raw_tx_rx_spark_mq_2t1r.yaml) (TX scaling), and [`_2t2r`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_raw_tx_rx_spark_mq_2t2r.yaml) (balanced). All run on `daqiri_bench_raw_gpudirect` at the native 8 KB shape.
    ...
    **Socket — RoCE (RDMA)** (`stream_type: "socket"`, `protocol: "roce"`) — runs on `daqiri_bench_rdma` (use `--mode {tx,rx,both}`). Configs use `kind: host_pinned` regardless of platform.
    ...
    - **DGX Spark netns wire loopback** (prefilled, split by role) — [`daqiri_bench_rdma_tx_rx_spark_netns_server.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_rdma_tx_rx_spark_netns_server.yaml) and [`daqiri_bench_rdma_tx_rx_spark_netns_client.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_rdma_tx_rx_spark_netns_client.yaml). Run each in its own network namespace (`--mode server` / `--mode client`) so RDMA-CM resolves over the wire; see [Socket and RDMA Benchmarking](../benchmarks/socket_benchmarking.md#run-the-rdma-roce-benchmark).
    ...
    **Socket — UDP / TCP** (`stream_type: "socket"`, `protocol: "udp"` or `"tcp"`) — runs on `daqiri_bench_socket`. The shipped smoke-test configs bind to `127.0.0.1`; see [Socket and RDMA Benchmarking](../benchmarks/socket_benchmarking.md) for namespace-based wire tests.
    ...
    - **UDP — DGX Spark netns wire loopback** (split by role) — [`daqiri_bench_socket_udp_tx_rx_netns_server.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_socket_udp_tx_rx_netns_server.yaml) and [`daqiri_bench_socket_udp_tx_rx_netns_client.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_socket_udp_tx_rx_netns_client.yaml). Run each in its own network namespace (`--mode server` / `--mode client`) so same-host IPs cross the wire instead of looping through `lo`; see [Socket and RDMA Benchmarking](../benchmarks/socket_benchmarking.md#run-the-linux-socket-benchmark).
    ...
    - **TCP — DGX Spark netns wire loopback** (split by role) — [`daqiri_bench_socket_tcp_tx_rx_netns_server.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_socket_tcp_tx_rx_netns_server.yaml) and [`daqiri_bench_socket_tcp_tx_rx_netns_client.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_socket_tcp_tx_rx_netns_client.yaml). Run each in its own network namespace (`--mode server` / `--mode client`); see [Socket and RDMA Benchmarking](../benchmarks/socket_benchmarking.md#run-the-linux-socket-benchmark).
