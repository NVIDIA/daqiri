...
8. :material-package-variant: **`num_bufs`** · `integer` · *required*: Number of buffers in the region. Higher gives more time to process packets but uses more BAR1 space. Too low risks NIC drops (RX) or a stalled TX path. A good starting point is 3×–5× the queue `batch_size`. For Raw Ethernet (`stream_type: "raw"`) a queue-backed region must cover the NIC descriptor ring *and* the TX burst headroom simultaneously, so the floor is `max(1.5 × ring, ring + 2 × batch_size)` — below it TX stalls with no error and the RX refill path starves. `daqiri_init` auto-bumps such regions to `max(3 × ring, ring + 4 × batch_size)` and logs a `WARN`. With the default 8192 ring and `batch_size: 10240` that is a floor of 28672 and a bump target of 49152; this file's 51200 already clears both.
