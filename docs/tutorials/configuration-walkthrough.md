...
??? question "1. I want to measure baseline throughput"
    ...
    **Raw Ethernet** (`stream_type: "raw"`) runs on `daqiri_bench_raw_gpudirect`.
    ...
    - **DGX Spark / GB10**: generate the loopback from the Spark ports, cores, RX MAC, and `kind: host_pinned` with [`gen_daqiri_config.py`](../config-generation.md#generate-a-raw-ethernet-pair). `examples/run_spark_bench.sh` does this for every benchmark cell.
    - **DGX Spark multi-queue core-scaling matrix**: `examples/run_spark_mq_bench.sh` supplies each 1×1, 1×2, 2×1, and 2×2 cell directly to the shared generator. No matrix superset is checked in or pruned.
    - **DGX Spark cross-host**: generate independent TX and RX roles with `raw-pair --role both`, copy the role file to its host, and start RX before TX. See [configuration generation](../config-generation.md#generate-a-raw-ethernet-pair) and the [cross-host network setup](../tutorials/system_configuration.md#cross-host-variant-two-sparks).
    ...
    **Raw Ethernet hardware tunnel transforms** run on `daqiri_bench_raw_gpudirect`. Generate them with the raw profile and `--transform vlan`, `vxlan`, `gre`, or `nvgre`; see [configuration generation](../config-generation.md#generate-a-raw-ethernet-pair).
    ...
    **Socket (RoCE / RDMA)** (`stream_type: "socket"`, `roce://` endpoints) runs on `daqiri_bench_rdma` (use `--mode {server,client,both}`). Configs use `kind: host_pinned` regardless of platform.
    ...
    - **DGX Spark netns or cross-host**: generate the concrete client and server files with `socket-pair --transport roce --role both`. `examples/run_spark_bench.sh` uses this path for its namespace sweep. See [configuration generation](../config-generation.md#generate-udp-tcp-or-roce-roles) and [Socket and RDMA Benchmarking](../benchmarks/socket_benchmarking.md#run-the-rdma-roce-benchmark).
    ...
    **Socket (UDP / TCP)** (`stream_type: "socket"` with `udp://` or `tcp://` endpoints) runs on `daqiri_bench_socket`. The shipped smoke-test configs bind to `127.0.0.1`. See [Socket and RDMA Benchmarking](../benchmarks/socket_benchmarking.md) for namespace-based wire tests.
    ...
    - **DGX Spark netns or cross-host**: generate one concrete file per role with `socket-pair --transport udp` or `--transport tcp`. The socket engine binds every interface in a file, so each host runs only its generated role. See [configuration generation](../config-generation.md#generate-udp-tcp-or-roce-roles).
