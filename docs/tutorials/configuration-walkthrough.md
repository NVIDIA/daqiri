...
20. The list of flows. Flows route packets to a queue based on packet fields. If `flows` is missing, all packets are routed to the first queue. Per interface, use only standard UDP/IP flows or only flex-item flows — not both.
