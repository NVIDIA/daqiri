...
??? question "1. I want to measure baseline throughput"
    ...
    **Raw Ethernet** (`stream_type: "raw"`) runs on `daqiri_bench_raw_gpudirect`.
    ...
    - **Runtime named sender with payload-only TX buffers** (template, replace `<placeholders>`): [`daqiri_example_named_sender_tx_rx.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_example_named_sender_tx_rx.yaml) (runs on `daqiri_example_named_sender`). Uses the raw ibverbs engine, creates the Ethernet/IPv4/UDP sender at runtime, and selects the TX queue at submission.
