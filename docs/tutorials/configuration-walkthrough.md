...
8. :material-package-variant: The number of buffers in the memory region. A higher value means more time to process the data, but takes additional space on the GPU BAR1. Too low increases the risk of dropping packets from the NIC having nowhere to write (Rx) or higher latency from buffering (Tx). A good starting point is 3x-5x the `batch_size` below. For the DPDK backend, `num_bufs` must additionally be at least 1.5x the NIC descriptor ring size (12288 with the default 8192 descriptors) — smaller pools deadlock the worker once the ring fills, so `daqiri_init` auto-bumps any below-threshold MR to 3x the ring size (24576) and emits a `WARN` naming the MR and the adjusted value.
