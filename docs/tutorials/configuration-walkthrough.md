...
??? question "1. I want to measure baseline throughput"
    ...
    **Raw Ethernet** (`stream_type: "raw"`) runs on `daqiri_bench_raw_gpudirect`.
    ...
    - **Accurate-send frame timing** (template, replace `<placeholders>`): [`daqiri_repro_accurate_send_frame.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_repro_accurate_send_frame.yaml) (runs on `daqiri_repro_accurate_send_frame`). Reproduces 60 Hz frames with 4,320 WAIT+SEND pairs, a 62-byte CPU header plus 1,440-byte GPU payload, one doorbell per frame, and writes per-packet hardware RX timings to CSV. `daqiri_repro_accurate_send_frame_inline64.yaml` retains the timing pattern with 64-byte host packets embedded in SEND WQEs.
