...
- **Socket (UDP / TCP)**: `stream_type: "socket"` with `udp://` or `tcp://` endpoints. Plain Linux kernel sockets. No NIC, no privileges, no special CMake flags. Useful as a comparison baseline and as a path to first results on a system without an NVIDIA NIC. Socket options are runtime API calls: resolve a connection ID, then pass native Linux constants to `socket_setsockopt()` rather than adding option names to YAML. The exception is the pair of kernel buffer sizes, `socket_config.rx_buffer_size` / `tx_buffer_size`, which have to be configuration because the sockets are already receiving before an application holds a connection ID.
...
??? question "1. I want to measure baseline throughput"
    ...
    **Raw Ethernet** (`stream_type: "raw"`) runs on `daqiri_bench_raw_gpudirect`.
    ...
    - **RTX PRO 6000 Blackwell header-data split**: [`daqiri_bench_raw_tx_rx_hds_rtx_pro_6000.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_raw_tx_rx_hds_rtx_pro_6000.yaml). Runs on `daqiri_bench_raw_hds` over the same wire loopback, with the header segment in CPU memory and the payload segment in GPU memory.
    - **RTX PRO 6000 Blackwell multi-queue core-scaling matrix**: one base config [`daqiri_bench_raw_tx_rx_rtx_pro_6000_mq.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_raw_tx_rx_rtx_pro_6000_mq.yaml) (the TX=2/RX=2 superset) from which `examples/run_rtx_pro_mq_bench.sh` (via `scripts/gen_rtx_pro_mq_config.py`) derives the four `(TX, RX)` cells (1,1), (1,2), (2,1), and (2,2) by pruning queues/flows, then sweeps payload sizes. All run on `daqiri_bench_raw_gpudirect`.
    - **RTX PRO 6000 Blackwell ibverbs (MPRQ) RX with a paired DPDK TX**: the RX side uses [`daqiri_bench_raw_rx_ibverbs_rtx_pro_6000.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_raw_rx_ibverbs_rtx_pro_6000.yaml) (`engine: ibverbs`, runs on `daqiri_bench_raw_gpudirect` built with `ibverbs`), fed over the loopback cable by a DPDK sender from [`daqiri_bench_raw_tx_only_rtx_pro_6000_nic.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_raw_tx_only_rtx_pro_6000_nic.yaml).
    ...
    **Socket (RoCE / RDMA)** (`stream_type: "socket"`, `roce://` endpoints) runs on `daqiri_bench_rdma` (use `--mode {tx,rx,both}`). Configs use `kind: host_pinned` regardless of platform.
    ...
    - **RTX PRO 6000 Blackwell wire loopback** (prefilled): [`daqiri_bench_rdma_tx_rx_rtx_pro_6000.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_rdma_tx_rx_rtx_pro_6000.yaml). Client and server sit on the two ends of the loopback cable. For a run that actually crosses the cable, put the two ports in separate namespaces first (`scripts/setup_spark_wire_loopback_netns.sh up`, which is not Spark-specific once `CLIENT_IF`/`SERVER_IF` are set): with both IPs in one namespace, RDMA-CM sees the peer as local and fails to resolve it. `examples/run_rtx_pro_bench.sh rdma` then runs the combined netns base per role, one namespace each. Measured rates: [Performance: RTX PRO 6000](../benchmarks/performance-rtx-pro-6000.md#socket-roce).
