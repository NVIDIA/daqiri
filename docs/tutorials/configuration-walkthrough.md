...
- **Raw — EFA (AWS)** — `stream_type: "raw"` with `engine: "efa"`. Kernel-bypass over the AWS [Elastic Fabric Adapter](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/efa.html) via libfabric (SRD), with GPUDirect. For EFA-enabled AWS instances (p4d/p5, etc.). Requires a build with `efa` in `DAQIRI_ENGINE` and the `efa` kernel driver on the host. Two-sided SEND/RECV only in this release.
...
(`DAQIRI_ENGINE` at the CMake layer selects which optional engine implementations to compile in — `dpdk` enables `stream_type: "raw"`, `ibverbs` enables `roce://` endpoints and the `engine: "ibverbs"` raw engine, and `efa` enables the `engine: "efa"` raw engine. Linux UDP/TCP sockets are always built in. The default build is `dpdk ibverbs`.)
...
??? question "1. I want to measure baseline throughput"
    ...
    **Raw — EFA (AWS)** (`stream_type: "raw"`, `engine: "efa"`) — runs on `daqiri_bench_rdma` (use `--mode {tx,rx,both}`), since EFA reuses the same two-sided SEND/RECV burst API. Requires a build with `efa` in `DAQIRI_ENGINE` and an EFA-enabled AWS instance with the `efa` kernel driver loaded (verify with `fi_info -p efa`).

    - **Generic** (template — replace IPs) — [`daqiri_bench_efa_tx_rx.yaml`](https://github.com/nvidia/daqiri/blob/main/examples/daqiri_bench_efa_tx_rx.yaml). Uses `kind: device` GPU memory regions to exercise GPUDirect (FI_HMEM); each interface's `efa_config` gives its server/client role and bootstrap TCP port.
