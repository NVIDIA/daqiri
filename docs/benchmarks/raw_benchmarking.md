...
| Dynamic resource lifecycle | Any ibverbs config with an RX queue and a single-region TX queue plus `daqiri_example_dynamic_resource` | Initializes without RX queues, creates the first RX queue and steering flow at runtime, adds a TX queue, then drains and removes the flows, queues, and owned regions |
