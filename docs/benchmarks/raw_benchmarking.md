...
| Dynamic resource lifecycle | Any ibverbs TX/RX config with `daqiri_example_dynamic_resource` | Clones the configured queue/MR geometry, adds owned RX and TX regions and queues at runtime, drains and removes both queues, then removes both regions |
