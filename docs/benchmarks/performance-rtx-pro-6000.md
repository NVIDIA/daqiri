...
| Raw Ethernet / GPUDirect | 8000 B packet | **390.5 Gb/s** (97.6%) | 373.1 | 0 |
| Raw Ethernet / ibverbs (MPRQ) | 8000 B packet | **390.4 Gb/s** (97.6%) | 260.8 | 0 |
| Raw Ethernet / header-data split | 8000 B payload | **389.0 Gb/s** (97.3%) | 375.2 | 0 |
...
is recorded as a failure rather than as a slow result. The unpaced UDP row is the
only one where the two rate columns are far apart: its sender put 46.3 Gb/s of
payload on the wire and the receiving application read 31.1, all of the difference
discarded inside the host *after* the frames were safely off the cable. That gap
...
RoCE's transmit engine runs entirely inside the NIC and leaves fewer gaps than a
software sender polling a queue, while one kernel thread per side cannot fill a
400 GbE pipe at all.
...
averaged in. Without it, a cell that lost its transmit core and only managed
...
The cable sustains **390.5 Gb/s**, 97.6% of the port. Over 20 s the sending port
put 115,783,681 frames on the wire and the receiving port took exactly
115,783,681 off, and the three repetitions held 390.5, 390.4 and 390.5 Gb/s with
...
| Payload | Cable Gb/s | App Gb/s | At batch |
| ------- | ---------: | -------: | -------- |
...
size. At 1024 B the gap is purely on the receive side: the cable carries 355.5 and
one receive core takes 259.2.
...
each pinned to its own core. Cells are derived from the single base
...
receive side: at 1024 B with two send queues the cable carried 372.2 while one
...
the matrix multiply and the cable still carries ~361 Gb/s while the application
...
The same traffic through the [ibverbs multi-packet receive queue](raw_benchmarking.md)
...
Every frame the sender emitted arrived, with no pause frames, so the shortfall is
inside the receive path and not on the link. This is an open item.

!!! note "Why this row's whole-run rate is a lower bound"
    ...
    measured window — dividing all the bytes by only the receiver's 20 s produced
    472 Gb/s on a 400 Gb/s port. The runner now checks every rate against line
    rate and falls back to the full snapshot window, which spans start-up and
...
9000-byte netdev MTU), and the receiving HCA places the bytes directly into the
server's pre-posted receive memory. Nothing runs per-packet on either CPU, which
is why this is the fastest backend on the page: **396.9 Gb/s sustained**, 99.2% of
the port, with 383.5 consumed.

## Socket / TCP

Ordinary Linux TCP, written in 1 MiB chunks, one kernel thread per side doing the
segmenting, checksums and copies. **64.3 Gb/s** on the cable and 63.2 consumed —
TCP self-paces, so the two track each other and nothing is lost.

This is 16% of the port with the largest frames on the page. One stream through
the kernel stack is simply not a 400 GbE transport; the comparison worth drawing
is against RoCE on the same cable, six times faster with smaller frames.
...
- **`ibverbs` RX consumes 260.8 Gb/s** while its own transmit side holds 390.4.
  Open item; the cable is not implicated.
- **The UDP receive ceiling is per-datagram cost in the socket engine**, which
  ...
  tracked separately from the benchmark harness.
- **The GPU-workload cells leave frames unconsumed** — the cable carries 361 Gb/s
...
with a `runs.csv` carrying `wire_tx_gbps` / `wire_rx_gbps` per cell, and a
`wire_validation.txt` holding the raw NIC frame and byte counts, what was lost,
...
./examples/run_rtx_pro_bench.sh dpdk-hds nic-smoke --seconds 30
./examples/run_rtx_pro_bench.sh ibverbs  nic-smoke --seconds 30
...
The host-specific record behind this page — the exact topology, the discovery
quirks, and the per-run evidence — is
[`examples/rtx_pro_6000_baseline.md`](https://github.com/nvidia/daqiri/blob/main/examples/rtx_pro_6000_baseline.md).
