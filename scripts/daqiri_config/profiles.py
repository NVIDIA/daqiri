...
RAW_MAX_FRAME_SIZE = 9100
RAW_TRANSFORM_WIRE_OVERHEAD = {
    "none": 0,
    "vlan": 4,
    "vxlan": 14 + 20 + 8 + 8,
    "gre": 14 + 20 + 4,
...
class RawPairSpec:
    ...
    def resolved_buffer_size(self) -> int:
        """Return the configured packet-buffer capacity in bytes."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.include_benchmark:
            ...
            if self.resolved_buffer_size < self.header_size + self.payload_size:
                raise ConfigError(
                    "raw buffer_size must be at least header_size + payload_size "
                    "for benchmark profiles"
                )
        wire_frame_size = (
            self.resolved_buffer_size + RAW_TRANSFORM_WIRE_OVERHEAD[self.transform]
        )
        if wire_frame_size > RAW_MAX_FRAME_SIZE:
            raise ConfigError(
                "raw buffer_size + transform overhead "
                f"({wire_frame_size} bytes) exceeds the runtime frame-size limit "
                f"of {RAW_MAX_FRAME_SIZE} bytes"
            )
    ...
    return [
        {
            ...
            "affinity": spec.affinity,
            "num_bufs": spec.num_bufs,
            "buf_size": spec.resolved_buffer_size,
        }
