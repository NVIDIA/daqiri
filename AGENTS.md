...
- `docs/benchmarks/` — benchmark guide pages, surfaced as a top-level "Benchmarking" nav section in `mkdocs.yml` and the landing page (`docs/index.md`):
  ...
  - `docs/benchmarks/reproducible-harness.md` — versioned suite/site manifests, safe local/SSH lifecycle, provenance artifacts, physical cross-host UDP acceptance, and canonical result states
