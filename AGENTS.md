...
### Segments & HDS
A single packet can span multiple **segments** (contiguous memory regions), each in CPU or GPU memory. The header-data split (HDS) mode puts headers in segment 0 (CPU) and payload in segment 1 (GPU), enabling GPUDirect zero-copy payload paths. Batched-GPU and CPU-only modes use a single segment. Raw Ethernet queues may reference at most four memory regions, matching `MAX_NUM_SEGS` and the fixed `BurstParams` segment arrays; configs exceeding this limit are rejected during DPDK initialization preflight.
