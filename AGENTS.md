...
- `DAQIRI_BUILD_UCX_GPU_EGRESS` — builds the raw-Ethernet → DAQIRI reorder → batched CUDA → UCX/UCP application when the umbrella is enabled (default `OFF`; use `BASE_TARGET=ucx`).
- `BUILD_TESTING` — builds available component tests when explicitly set to `ON`, including the ibverbs reorder tracker and the UCX application's tests when that application is enabled.
...
| `daqiri_ucx_gpu_transport_bench`, `daqiri_ucx_raw_source`, `daqiri_ucx_raw_pipeline` | `applications/ucx_gpu_egress/` | UCX-only and composed raw-Ethernet → DAQIRI reorder → 16-image CUDA batch → single UCP Active Message paths. Templates are under `configs/`; use `BASE_TARGET=ucx`, disable the ResNet sub-option, and follow `docs/tutorials/daqiri-ucx-gpu-egress.md`. |
...
For host-visible raw RX memory, the ibverbs GPU reorder path checks source-batch
boundaries before count-based accumulation. It drops duplicate and stale packets
and closes an incomplete batch when a newer batch arrives. Device-memory RX keeps
the legacy count-only behavior because the poller cannot read its packet headers.
