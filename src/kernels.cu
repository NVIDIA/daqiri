 ...
 */
__global__ void
simple_packet_reorder_kernel(void *__restrict__ out,
  ...
  if (pkt_idx < num_pkts) {
    for (int pos = threadIdx.x; pos < len / 4; pos += blockDim.x) {
      ...
      uint32_t *out_ptr =
          (uint32_t *)((uint8_t *)out + pkt_idx * pkt_len) + pos;
      *out_ptr = *in_ptr;
    }
  }
}
 ...
 */
extern "C" void simple_packet_reorder(void *out, const void *const *const in,
                                      uint16_t pkt_len, uint32_t num_pkts,
                                      cudaStream_t stream) {
  simple_packet_reorder_kernel<<<num_pkts, 128, 0, stream>>>(out, in, pkt_len,
                                                             num_pkts);
}

__device__ static inline uint32_t
extract_bits_be(const uint8_t *data, uint16_t bit_offset, uint8_t bit_width) {
  uint32_t value = 0;
...
__device__ static inline uint16_t bswap16(uint16_t value) {
  return static_cast<uint16_t>(
      __byte_perm(static_cast<uint32_t>(value), 0U, 0x4401U));
}
...
template <uint8_t InputType> struct ReorderInputReader;

template <> struct ReorderInputReader<kReorderDataTypeInt4> {
  static constexpr uint32_t kBits = 4;

  __device__ static inline int32_t
  read(const uint8_t *src, uint32_t element_idx, uint8_t input_endianness) {
    (void)input_endianness;
    const uint8_t packed = src[element_idx >> 1U];
    const uint8_t nibble = (element_idx & 0x1U) == 0U
...
template <> struct ReorderInputReader<kReorderDataTypeInt8> {
  ...
  read(const uint8_t *src, uint32_t element_idx, uint8_t input_endianness) {
    ...
    return static_cast<int32_t>(
        reinterpret_cast<const int8_t *>(src)[element_idx]);
  }
};

template <> struct ReorderInputReader<kReorderDataTypeInt16> {
  static constexpr uint32_t kBits = 16;

  __device__ static inline int32_t
  read(const uint8_t *src, uint32_t element_idx, uint8_t input_endianness) {
    const uint32_t byte_idx = element_idx * 2U;
    uint16_t value = static_cast<uint16_t>(src[byte_idx]) |
                     (static_cast<uint16_t>(src[byte_idx + 1U]) << 8U);
    if (is_network_endianness(input_endianness)) {
      value = bswap16(value);
    }
...
template <> struct ReorderInputReader<kReorderDataTypeInt32> {
...
template <uint8_t OutputType> struct ReorderOutputWriter;

template <> struct ReorderOutputWriter<kReorderDataTypeFp16> {
  __device__ static inline void write(uint8_t *dst, uint32_t element_idx,
                                      int32_t value) {
    reinterpret_cast<__half *>(dst)[element_idx] =
        __float2half_rn(static_cast<float>(value));
  }
};

template <> struct ReorderOutputWriter<kReorderDataTypeBf16> {
  __device__ static inline void write(uint8_t *dst, uint32_t element_idx,
                                      int32_t value) {
...
template <> struct ReorderOutputWriter<kReorderDataTypeFp32> {
  __device__ static inline void write(uint8_t *dst, uint32_t element_idx,
                                      int32_t value) {
...
template <> struct ReorderOutputWriter<kReorderDataTypeFp64> {
  __device__ static inline void write(uint8_t *dst, uint32_t element_idx,
                                      int32_t value) {
...
template <> struct ReorderOutputWriter<kReorderDataTypeInt32> {
  __device__ static inline void write(uint8_t *dst, uint32_t element_idx,
                                      int32_t value) {
...
convert_reorder_value(uint8_t *dst, const uint8_t *src, uint32_t element_idx,
                      uint8_t input_endianness) {
  const int32_t value =
      ReorderInputReader<InputType>::read(src, element_idx, input_endianness);
  ReorderOutputWriter<OutputType>::write(dst, element_idx, value);
}

template <uint8_t InputAlignment>
__device__ static inline uint32_t
load_u32_aligned_or_bytes(const uint8_t *src, const uint8_t *read_floor,
...
struct ReorderPayloadConverter<kReorderDataTypeInt4, OutputType,
                               InputAlignment> {
  __device__ static inline void convert(uint8_t *__restrict__ dst,
    ...
    for (uint32_t word_idx = static_cast<uint32_t>(threadIdx.x);
         word_idx < word_count; word_idx += blockDim.x) {
      const uint32_t word = load_u32_aligned_or_bytes<InputAlignment>(
          src + (word_idx * 4U), read_floor, read_end);
      const uint32_t base_element_idx = word_idx * 8U;

#pragma unroll
      for (uint32_t byte_idx = 0; byte_idx < 4U; ++byte_idx) {
        const uint8_t byte =
            static_cast<uint8_t>((word >> (byte_idx * 8U)) & 0xFFU);
        const uint32_t element_idx = base_element_idx + (byte_idx * 2U);
        ReorderOutputWriter<OutputType>::write(dst, element_idx,
                                               sign_extend_4(byte >> 4U));
        ReorderOutputWriter<OutputType>::write(dst, element_idx + 1U,
                                               sign_extend_4(byte));
      }
    }
    ...
    for (uint32_t element_idx =
             first_tail_element + static_cast<uint32_t>(threadIdx.x);
         element_idx < element_count; element_idx += blockDim.x) {
...
template <uint8_t OutputType, uint8_t InputAlignment>
struct ReorderPayloadConverter<kReorderDataTypeInt8, OutputType,
                               InputAlignment> {
  __device__ static inline void convert(uint8_t *__restrict__ dst,
                                        const uint8_t *__restrict__ src,
                                        const uint8_t *__restrict__ read_floor,
...
#pragma unroll
      for (uint32_t byte_idx = 0; byte_idx < 4U; ++byte_idx) {
        const int32_t value = static_cast<int32_t>(
            static_cast<int8_t>((word >> (byte_idx * 8U)) & 0xFFU));
        ReorderOutputWriter<OutputType>::write(dst, base_element_idx + byte_idx,
                                               value);
      }
    }
    ...
    for (uint32_t element_idx =
             first_tail_element + static_cast<uint32_t>(threadIdx.x);
         element_idx < input_payload_len; element_idx += blockDim.x) {
...
template <uint8_t OutputType, uint8_t InputAlignment>
struct ReorderPayloadConverter<kReorderDataTypeInt16, OutputType,
                               InputAlignment> {
  __device__ static inline void convert(uint8_t *__restrict__ dst,
                                        const uint8_t *__restrict__ src,
                                        const uint8_t *__restrict__ read_floor,
    ...
    for (uint32_t word_idx = static_cast<uint32_t>(threadIdx.x);
      ...
      const uint16_t second_raw =
          static_cast<uint16_t>((ordered_word >> 16U) & 0xFFFFU);
      const int32_t first_value =
          static_cast<int32_t>(static_cast<int16_t>(first_raw));
      const int32_t second_value =
          static_cast<int32_t>(static_cast<int16_t>(second_raw));
      ReorderOutputWriter<OutputType>::write(dst, base_element_idx,
                                             first_value);
      ReorderOutputWriter<OutputType>::write(dst, base_element_idx + 1U,
                                             second_value);
    }
...
template <uint8_t OutputType, uint8_t InputAlignment>
struct ReorderPayloadConverter<kReorderDataTypeInt32, OutputType,
                               InputAlignment> {
  __device__ static inline void convert(uint8_t *__restrict__ dst,
                                        const uint8_t *__restrict__ src,
                                        const uint8_t *__restrict__ read_floor,
    ...
    for (uint32_t word_idx = static_cast<uint32_t>(threadIdx.x);
      ...
      const uint32_t value =
          is_network_endianness(input_endianness) ? bswap32(word) : word;
      ReorderOutputWriter<OutputType>::write(dst, word_idx,
...
template <uint8_t InputType, uint8_t OutputType, uint8_t InputAlignment>
__device__ static inline void convert_reorder_payload_typed(
    uint8_t *__restrict__ dst, const uint8_t *__restrict__ src,
    const uint8_t *__restrict__ read_floor,
    const uint8_t *__restrict__ read_end, uint8_t input_endianness,
    uint32_t input_payload_len) {
...
__device__ static inline void convert_reorder_payload_for_input(
    uint8_t *__restrict__ dst, const uint8_t *__restrict__ src,
    const uint8_t *__restrict__ read_floor,
    const uint8_t *__restrict__ read_end, uint8_t input_endianness,
    uint32_t input_payload_len, uint8_t output_data_type) {
  switch (output_data_type) {
  case kReorderDataTypeFp16:
    convert_reorder_payload_typed<InputType, kReorderDataTypeFp16,
                                  InputAlignment>(
        dst, src, read_floor, read_end, input_endianness, input_payload_len);
    break;
  case kReorderDataTypeBf16:
    convert_reorder_payload_typed<InputType, kReorderDataTypeBf16,
                                  InputAlignment>(
        dst, src, read_floor, read_end, input_endianness, input_payload_len);
    break;
  case kReorderDataTypeFp32:
    convert_reorder_payload_typed<InputType, kReorderDataTypeFp32,
                                  InputAlignment>(
        dst, src, read_floor, read_end, input_endianness, input_payload_len);
    break;
  case kReorderDataTypeFp64:
    convert_reorder_payload_typed<InputType, kReorderDataTypeFp64,
                                  InputAlignment>(
        dst, src, read_floor, read_end, input_endianness, input_payload_len);
    break;
  case kReorderDataTypeInt32:
    convert_reorder_payload_typed<InputType, kReorderDataTypeInt32,
                                  InputAlignment>(
        dst, src, read_floor, read_end, input_endianness, input_payload_len);
    break;
  default:
...
__device__ static inline void convert_reorder_payload_for_alignment(
    uint8_t *__restrict__ dst, const uint8_t *__restrict__ src,
    uint32_t input_payload_len, uint8_t input_data_type,
    uint8_t output_data_type, uint8_t input_endianness,
    const uint8_t *__restrict__ read_floor) {
...
__device__ static inline void
convert_reorder_payload(uint8_t *__restrict__ dst,
                        const uint8_t *__restrict__ src,
                        uint32_t input_payload_len, uint8_t input_data_type,
                        uint8_t output_data_type, uint8_t input_endianness,
                        const uint8_t *__restrict__ read_floor) {
  ...
  case 0U:
    convert_reorder_payload_for_alignment<0U>(dst, src, input_payload_len,
                                              input_data_type, output_data_type,
                                              input_endianness, read_floor);
    break;
  case 1U:
    convert_reorder_payload_for_alignment<1U>(dst, src, input_payload_len,
                                              input_data_type, output_data_type,
                                              input_endianness, read_floor);
    break;
  case 2U:
    convert_reorder_payload_for_alignment<2U>(dst, src, input_payload_len,
                                              input_data_type, output_data_type,
                                              input_endianness, read_floor);
    break;
  default:
    convert_reorder_payload_for_alignment<3U>(dst, src, input_payload_len,
                                              input_data_type, output_data_type,
                                              input_endianness, read_floor);
    break;
  }
...
template <typename T>
__device__ static inline uint32_t
copy_vector_chunks(uint8_t *__restrict__ dst, const uint8_t *__restrict__ src,
                   uint32_t len) {
  ...
  for (uint32_t pos = static_cast<uint32_t>(threadIdx.x); pos < count;
       pos += blockDim.x) {
...
__device__ static inline void
copy_payload_vectorized(uint8_t *__restrict__ dst,
                        const uint8_t *__restrict__ src, uint32_t len) {
  const uintptr_t src_addr = reinterpret_cast<uintptr_t>(src);
  ...
  if ((alignment & 0xFU) == 0U) {
    copied +=
        copy_vector_chunks<uint4>(dst + copied, src + copied, len - copied);
  }
  if (((alignment | static_cast<uintptr_t>(copied)) & 0x7U) == 0U) {
    copied +=
        copy_vector_chunks<uint2>(dst + copied, src + copied, len - copied);
  }
  if (((alignment | static_cast<uintptr_t>(copied)) & 0x3U) == 0U) {
    copied +=
        copy_vector_chunks<uint32_t>(dst + copied, src + copied, len - copied);
  }

  for (uint32_t pos = copied + static_cast<uint32_t>(threadIdx.x); pos < len;
       pos += blockDim.x) {
    dst[pos] = src[pos];
  }
}
...
__global__ void packet_reorder_copy_payload_by_sequence_kernel(
    void *__restrict__ out, const void *const *const __restrict__ in,
    uint32_t input_payload_len, uint32_t output_payload_len,
    uint32_t payload_byte_offset, uint32_t num_pkts, uint16_t seq_bit_offset,
    uint8_t seq_bit_width, uint16_t batch_bit_offset, uint8_t batch_bit_width,
    uint8_t has_batch_number, uint32_t packets_per_batch, uint32_t max_slot_idx,
    uint8_t input_data_type, uint8_t output_data_type, uint8_t input_endianness,
    ...
    const uint64_t batch_id =
        (has_batch_number != 0U)
            ? static_cast<uint64_t>(
                  extract_bits_be(src_pkt, batch_bit_offset, batch_bit_width))
  ...
  if (input_data_type == kReorderDataTypeSame ||
      output_data_type == kReorderDataTypeSame) {
    copy_payload_vectorized(dst, src, input_payload_len);
    return;
  }

  convert_reorder_payload(dst, src, input_payload_len, input_data_type,
                          output_data_type, input_endianness, src_pkt);
}

extern "C" void packet_reorder_copy_payload_by_sequence(
    void *out, const void *const *const in, uint32_t input_payload_len,
    uint32_t output_payload_len, uint32_t payload_byte_offset,
    uint32_t num_pkts, uint16_t seq_bit_offset, uint8_t seq_bit_width,
    uint16_t batch_bit_offset, uint8_t batch_bit_width,
    uint8_t has_batch_number, uint32_t packets_per_batch, uint32_t max_slot_idx,
    uint8_t input_data_type, uint8_t output_data_type, uint8_t input_endianness,
    uint64_t *batch_id_out, cudaStream_t stream) {
  if (out == nullptr || in == nullptr || input_payload_len == 0 ||
      output_payload_len == 0 || num_pkts == 0 || packets_per_batch == 0) {
    return;
  }

  packet_reorder_copy_payload_by_sequence_kernel<<<num_pkts, 128, 0, stream>>>(
      out, in, input_payload_len, output_payload_len, payload_byte_offset,
      num_pkts, seq_bit_offset, seq_bit_width, batch_bit_offset,
      batch_bit_width, has_batch_number, packets_per_batch, max_slot_idx,
      input_data_type, output_data_type, input_endianness, batch_id_out);
}
