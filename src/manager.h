...
public:
  ...
  virtual void *get_segment_packet_ptr(BurstParams *burst, int seg,
                                       int idx) = 0;
  virtual uint32_t get_segment_packet_length(BurstParams *burst, int seg,
                                             int idx) = 0;
  ...
  virtual Status set_eth_header(BurstParams *burst, int idx,
                                char *dst_addr) = 0;
  virtual Status set_ipv4_header(BurstParams *burst, int idx, int ip_len,
  ...
  virtual Status set_udp_payload(BurstParams *burst, int idx, void *data,
                                 int len) = 0;
  ...
  virtual Status set_packet_tx_time(BurstParams *burst, int idx,
                                    uint64_t time) = 0;
  virtual void shutdown() = 0;
  virtual void print_stats() = 0;
  virtual uint64_t get_burst_tot_byte(BurstParams *burst) = 0;
  virtual BurstParams *create_tx_burst_params() = 0;
  virtual Status get_rx_burst(BurstParams **burst, uintptr_t conn_id,
                              bool server);
  ...
  virtual Status set_reorder_cuda_stream(const std::string &interface_name,
                                         ...
                                         cudaStream_t stream);
  virtual Status get_reorder_burst_info(BurstParams *burst,
  ...
  virtual int
  get_port_id(const std::string &key) final; // NOLINT(readability/inheritance)
  ...
  virtual Status socket_connect_to_server(const std::string &dst_addr,
                                          uint16_t dst_port,
                                          uintptr_t *conn_id);
  virtual Status socket_connect_to_server(const std::string &dst_addr,
                                          uint16_t dst_port,
                                          ...
                                          uintptr_t *conn_id);
  virtual Status socket_get_port_queue(uintptr_t conn_id, uint16_t *port,
                                       uint16_t *queue);
  virtual Status socket_get_server_conn_id(const std::string &server_addr,
                                           uint16_t server_port,
                                           uintptr_t *conn_id);

  virtual Status rdma_connect_to_server(const std::string &dst_addr,
                                        uint16_t dst_port, uintptr_t *conn_id);
  virtual Status rdma_connect_to_server(const std::string &dst_addr,
                                        uint16_t dst_port,
                                        const std::string &src_addr,
                                        uintptr_t *conn_id);
  virtual Status rdma_get_port_queue(uintptr_t conn_id, uint16_t *port,
                                     uint16_t *queue);
  virtual Status rdma_get_server_conn_id(const std::string &server_addr,
                                         uint16_t server_port,
                                         uintptr_t *conn_id);
  virtual Status rdma_set_header(BurstParams *burst, RDMAOpCode op_code,
  ...
  struct rte_mempool *create_pktmbuf_pool(const std::string &name,
                                          const MemoryRegionConfig &mr);
  struct rte_mempool *create_generic_pool(const std::string &name,
                                          const MemoryRegionConfig &mr);
...
protected:
  ...
  std::unordered_map<std::string, std::shared_ptr<struct rte_pktmbuf_extmem>>
      ext_pktmbufs_;
  std::unordered_map<uint32_t, std::vector<std::pair<uint16_t, uint16_t>>>
      rx_core_q_map;
  ...
  std::unordered_map<int, size_t>
      next_queue_index_map_; // For get_rx_burst next queue check
  ...
  static void set_manager_type(ManagerType type) {
    if (ManagerType_ != ManagerType::UNKNOWN && ManagerType_ != type) {
      throw std::logic_error(
          "Manager type is already set with another manager type.");
    }
