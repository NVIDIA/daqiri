 ...
 */
class IbverbsMgr : public Manager {
 public:
  IbverbsMgr() = default;
  ~IbverbsMgr() override;
  ...
  Status set_reorder_cuda_stream(const std::string& interface_name,
                                 const std::string& reorder_name, cudaStream_t stream) override;
 ...
 private:
  ...
  Status setup_rx_queue(IbvRxQueue& q, const InterfaceConfig& intf,
  ...
  void reorder_poll_events(IbvRxQueue& q, IbvReorderPlan& plan);  // free sources of finished batches
