...
#include "src/managers/ibverbs/mlx5_prm_min.h"
...
static inline uint16_t rdr_seq_off(const ReorderConfig& c) {
  return c.method_ == ReorderMethod::SEQ_BATCH_NUMBER ? c.seq_batch_number_.sequence_number_.bit_offset_
                                                      : c.seq_packets_per_batch_.sequence_number_.bit_offset_;
}
static inline uint8_t rdr_seq_width(const ReorderConfig& c) {
  return c.method_ == ReorderMethod::SEQ_BATCH_NUMBER ? c.seq_batch_number_.sequence_number_.bit_width_
                                                      : c.seq_packets_per_batch_.sequence_number_.bit_width_;
}
static inline uint16_t rdr_batch_off(const ReorderConfig& c) {
  return c.method_ == ReorderMethod::SEQ_BATCH_NUMBER ? c.seq_batch_number_.batch_number_.bit_offset_ : 0U;
}
static inline uint8_t rdr_batch_width(const ReorderConfig& c) {
  return c.method_ == ReorderMethod::SEQ_BATCH_NUMBER ? c.seq_batch_number_.batch_number_.bit_width_ : 0U;
}
static inline bool rdr_uses_conversion(const ReorderConfig& c) {
  if (!c.data_types_.enabled_) { return false; }
  if (c.data_types_.input_type_ != c.data_types_.output_type_) { return true; }
...
static inline uint32_t rdr_output_payload_len(const ReorderConfig& c, uint32_t in_len) {
  if (!rdr_uses_conversion(c)) { return in_len; }
...
IbverbsMgr::~IbverbsMgr() { shutdown(); }

// ---------------------------------------------------------------------------
// Bring-up
// ---------------------------------------------------------------------------
bool IbverbsMgr::set_config_and_initialize(const NetworkConfig& cfg) {
  cfg_ = cfg;
  initialize();
  return initialized_;
}

bool IbverbsMgr::init_eal() {
...
struct ibv_context* IbverbsMgr::open_device_for_interface(const InterfaceConfig& intf) {
  int num = 0;
  ...
  if (!intf.address_.empty()) { dev = open_named(intf.address_); }
  if (dev == nullptr && !intf.name_.empty()) { dev = open_named(intf.name_); }
...
int IbverbsMgr::mr_access_to_ibv(uint32_t access) {
  (void)access;
...
Status IbverbsMgr::register_mr(struct ibv_pd* pd, const std::string& mr_name, uint8_t** out_base,
                               uint32_t* out_lkey) {
  ...
  if (mr.kind_ == MemoryKind::DEVICE) {
    ...
    int dmabuf_fd = -1;
    CUresult cres = cuMemGetHandleForAddressRange(
        &dmabuf_fd, static_cast<CUdeviceptr>(aligned), aligned_size,
...
Status IbverbsMgr::register_rx_mr(IbvRxQueue& q) {
  ...
  for (const std::string& name : q.mr_names) {
    ...
    if (Status s = register_mr(q.pd, name, &r.base, &r.lkey); s != Status::SUCCESS) { return s; }
...
Status IbverbsMgr::create_striding_rq(IbvRxQueue& q) {
  if (!q.striding) { return create_regular_rq(q); }
  ...
  while ((1u << stride_log) < mr.buf_size_) { ++stride_log; }
  ...
  if (!q.striding && fit > 4096) { fit = 4096; }
  ...
  for (uint32_t i = 0; i < q.num_wqe; i++) { q.freed_strides[i].store(0); }
  ...
  if (Status s = create_striding_rq_devx(q); s != Status::SUCCESS) { return s; }
...
Status IbverbsMgr::create_regular_rq(IbvRxQueue& q) {
  ...
  for (const auto& r : q.regions) {
    seglog += " [seg_len=" + std::to_string(r.seg_len) + " slot=" + std::to_string(r.slot_size) +
              "]";
  }
...
Status IbverbsMgr::repost_wqe(IbvRxQueue& q, uint32_t wqe_idx) {
  struct ibv_sge sge{};
...
Status IbverbsMgr::post_all_wqes(IbvRxQueue& q) {
  for (uint32_t i = 0; i < q.num_wqe; i++) {
    if (repost_wqe(q, i) != Status::SUCCESS) { return Status::GENERIC_FAILURE; }
  }
  q.cur_wqe = 0;
  q.cur_consumed = 0;
  return Status::SUCCESS;
...
void IbverbsMgr::devx_build_wqe(IbvRxQueue& q, uint32_t slot) {
...
void IbverbsMgr::devx_ring_rq_doorbell(IbvRxQueue& q) {
  doorbell_store_barrier();
  q.rq_dbr[0] = htobe32(q.rq_pi & 0xffff);  // MLX5_RCV_DBR = index 0
}

void IbverbsMgr::devx_advance_producer(IbvRxQueue& q) {
  ...
  if (advanced) { devx_ring_rq_doorbell(q); }
}

Status IbverbsMgr::devx_create_rq(IbvRxQueue& q, uint32_t stride_log, uint32_t strides_log) {
  // PD number.
  ...
  DEVX_SET(wq, wq, wq_type,
           q.striding ? MLX5_WQ_TYPE_CYCLIC_STRIDING_RQ : MLX5_WQ_TYPE_CYCLIC);
  ...
  for (uint32_t i = 0; i < q.num_wqe; i++) { devx_build_wqe(q, i); }
  q.rq_pi = q.num_wqe;
...
Status IbverbsMgr::devx_create_tir(IbvRxQueue& q) {
...
bool IbverbsMgr::probe_send_scheduling(struct ibv_context* ctx) {
  ...
  if (mlx5dv_devx_general_cmd(ctx, in, sizeof(in), out.data(),
                              out.size() * sizeof(uint32_t)) != 0) {
...
struct mlx5dv_devx_obj* IbverbsMgr::create_flex_parser_node(struct ibv_context* ctx,
                                                            uint8_t arc_node, uint16_t compare_value,
                                                            uint16_t offset,
  ...
  *out_sample_id = DEVX_GET(parse_graph_flow_match_sample, q_sample, flow_match_sample_field_id);
  DAQIRI_LOG_INFO("Flex parser node created: id {} arc {} compare {} offset {} -> sample_field_id {}",
                  node_id, arc_node, compare_value, offset, *out_sample_id);
  return obj;
}

Status IbverbsMgr::install_port_flows() {
  for (const auto& intf : cfg_.ifs_) {
    ...
    for (const auto& fi : intf.rx_.flex_items_) {
      ...
      struct mlx5dv_devx_obj* node =
          create_flex_parser_node(ctx, MLX5_GRAPH_ARC_NODE_UDP, fi.udp_dst_port_, fi.offset_,
                                  &sample_id);
      if (node == nullptr) {
    ...
    for (const auto& fl : intf.rx_.flows_) {
      if (fl.action_.type_ != FlowType::QUEUE) { continue; }
      ...
      DAQIRI_LOG_INFO("Flow '{}' -> queue {} (port {}): udp_src={} udp_dst={} ipv4_len={}", fl.name_,
                      fl.action_.id_, port, mt.udp_src_, mt.udp_dst_, mt.ipv4_len_);
      installed++;
    }
...
Status IbverbsMgr::devx_create_cq(IbvRxQueue& q) {
  const size_t page = sysconf(_SC_PAGESIZE);
...
Status IbverbsMgr::create_striding_rq_devx(IbvRxQueue& q) {
  ...
  if (Status s = devx_create_cq(q); s != Status::SUCCESS) { return s; }
  if (Status s = devx_create_rq(q, stride_log, strides_log); s != Status::SUCCESS) { return s; }
  if (Status s = devx_create_tir(q); s != Status::SUCCESS) { return s; }
  // Flow steering is installed per-port (install_port_flows) once every RX queue
  // exists, so a flow can target any queue's TIR.
  return Status::SUCCESS;
}

void IbverbsMgr::devx_destroy(IbvRxQueue& q) {
  ...
  // here.
  if (q.dr_action) { mlx5dv_dr_action_destroy(q.dr_action); q.dr_action = nullptr; }
  if (q.tir_obj) { mlx5dv_devx_obj_destroy(q.tir_obj); q.tir_obj = nullptr; }
  if (q.rq_obj) { mlx5dv_devx_obj_destroy(q.rq_obj); q.rq_obj = nullptr; }
  if (q.cq_obj) { mlx5dv_devx_obj_destroy(q.cq_obj); q.cq_obj = nullptr; }
  if (q.td_obj) { mlx5dv_devx_obj_destroy(q.td_obj); q.td_obj = nullptr; }
  if (q.wq_umem) { mlx5dv_devx_umem_dereg(q.wq_umem); q.wq_umem = nullptr; }
  if (q.cq_umem) { mlx5dv_devx_umem_dereg(q.cq_umem); q.cq_umem = nullptr; }
  if (q.dbr_umem) { mlx5dv_devx_umem_dereg(q.dbr_umem); q.dbr_umem = nullptr; }
  if (q.devx_uar) { mlx5dv_devx_free_uar(q.devx_uar); q.devx_uar = nullptr; }
  // rq_dbr points inside wq_buf (single umem); freed with it.
  if (q.wq_buf) { free(q.wq_buf); q.wq_buf = nullptr; }
  if (q.cq_buf) { free(q.cq_buf); q.cq_buf = nullptr; }
  q.rq_dbr = nullptr;
}

Status IbverbsMgr::setup_rx_queue(IbvRxQueue& q, const InterfaceConfig& intf,
  ...
  if (!qcfg.common_.cpu_core_.empty()) { q.cpu_core = std::stoi(qcfg.common_.cpu_core_); }
  ...
  if (const char* s = getenv("DAQIRI_IBV_STRIDING")) { q.striding = (s[0] != '0'); }
  ...
  if (q.ctx == nullptr) { return Status::GENERIC_FAILURE; }
  q.pd = pd_map_[q.ctx];

  if (Status s = register_rx_mr(q); s != Status::SUCCESS) { return s; }
  if (Status s = create_striding_rq(q); s != Status::SUCCESS) { return s; }
  ...
  if (Status s = init_reorder(q, intf, qcfg); s != Status::SUCCESS) { return s; }

  return Status::SUCCESS;
}

void IbverbsMgr::initialize() {
  ...
  int if_num = 0;
  for (auto& intf : cfg_.ifs_) { intf.port_id_ = if_num++; }
  ...
  // on that device's TX queues.
  {
    ...
    for (auto& q : tx_queues_) {
      if (q->ctx == nullptr) { continue; }
      auto it = probed.find(q->ctx);
      if (it == probed.end()) { it = probed.emplace(q->ctx, probe_send_scheduling(q->ctx)).first; }
...
void IbverbsMgr::run() {
  ...
  for (auto& [core, grp] : rx_groups) { rx_threads.push_back(grp); }
  ...
  for (auto& [core, grp] : tx_groups) { tx_threads.push_back(grp); }
...
// RX hot path
// ---------------------------------------------------------------------------
bool IbverbsMgr::rx_alloc_burst(IbvRxQueue* q) {
...
void IbverbsMgr::rx_flush_burst(IbvRxQueue* q) {
  if (q->cur_burst == nullptr || q->cur_n == 0) { return; }
  ...
  if (rte_ring_enqueue(q->ring, q->cur_burst) != 0) {
    ...
    for (int i = 0; i < q->cur_n; i++) { release_strides(*q, wqe_arr[i], strd_arr[i]); }
...
void IbverbsMgr::rx_poll_queue(IbvRxQueue* q) {
  ...
  while (processed < budget) {
    ...
    q->cur_n++;
    if (q->cur_n == 1) { q->last_flush_tsc = rte_get_timer_cycles(); }
  ...
  if (q->cq_ci != start_ci) { *q->dv_cq.dbrec = htobe32(q->cq_ci & 0xffffff); }
}

void IbverbsMgr::rx_worker(std::vector<IbvRxQueue*> group) {
  ...
  while (leader->running.load(std::memory_order_relaxed) &&
    ...
    for (auto* q : group) { rx_poll_queue(q); }
  }
...
void IbverbsMgr::release_strides(IbvRxQueue& q, uint32_t wqe_idx, uint32_t strd) {
...
// ---------------------------------------------------------------------------
IbvRxQueue* IbverbsMgr::find_rx_queue(int port, int q) {
  for (auto& rq : rx_queues_) {
    if (rq->port_id == port && rq->queue_id == q) { return rq.get(); }
  }
  return nullptr;
}
...
// ---------------------------------------------------------------------------
Status IbverbsMgr::init_reorder(IbvRxQueue& q, const InterfaceConfig& intf,
                                const RxQueueConfig& qcfg) {
  if (intf.rx_.reorder_configs_.empty()) { return Status::SUCCESS; }
  ...
  for (const auto& fl : intf.rx_.flows_) { flow_to_queue[fl.id_] = fl.action_.id_; }

  auto st = std::make_unique<IbvReorderState>();
  for (const auto& rc : intf.rx_.reorder_configs_) {
    if (rc.reorder_type_ != "gpu") { continue; }
    ...
    for (uint16_t fid : rc.flow_ids_) {
      if (flow_to_queue[fid] == static_cast<uint16_t>(q.queue_id)) { st->flow_to_plan[fid] = plan_idx; }
    }
  ...
  if (st->plans.empty()) { return Status::SUCCESS; }
...
void IbverbsMgr::reorder_poll_events(IbvRxQueue& q, IbvReorderPlan& plan) {
  for (auto& ob : plan.out_bufs) {
    if (ob.event_complete || ob.event == nullptr) { continue; }
    if (cudaEventQuery(ob.event) != cudaSuccess) { continue; }  // still running
    // Kernel finished reading the source packets -> release their strides.
    for (uint32_t i = 0; i < ob.src_count; i++) { release_strides(q, ob.src_wqe[i], ob.src_strd[i]); }
    ob.src_wqe.clear();
    ob.src_strd.clear();
    ob.src_count = 0;
...
Status IbverbsMgr::reorder_flush_batch(IbvRxQueue& q, IbvReorderPlan& plan, BurstParams** out) {
  *out = nullptr;
  const uint32_t num_pkts = static_cast<uint32_t>(plan.acc_ptrs.size());
  if (num_pkts == 0) { return Status::SUCCESS; }
  ...
  if (idx == plan.out_bufs.size()) {
    ...
    for (uint32_t i = 0; i < num_pkts; i++) { release_strides(q, plan.acc_wqe[i], plan.acc_strd[i]); }
    plan.acc_ptrs.clear();
    plan.acc_wqe.clear();
    plan.acc_strd.clear();
  ...
  packet_reorder_copy_payload_by_sequence(
      ...
      rdr_batch_off(c), rdr_batch_width(c),
      c.method_ == ReorderMethod::SEQ_BATCH_NUMBER ? 1U : 0U, plan.packets_per_batch,
      plan.packets_per_batch - 1U,
...
void IbverbsMgr::reorder_process_raw(IbvRxQueue& q, BurstParams* raw) {
  auto& st = *q.reorder;
...
Status IbverbsMgr::reorder_get_rx(IbvRxQueue& q, BurstParams** burst) {
  auto& st = *q.reorder;
  std::lock_guard<std::mutex> guard(st.lock);
  for (auto& plan : st.plans) { reorder_poll_events(q, plan); }
  if (!st.ready.empty()) {
...
void IbverbsMgr::reorder_release_output(BurstParams* burst) {
  if (burst == nullptr || burst->custom_pkt_data == nullptr) { return; }
  auto ctx = std::static_pointer_cast<IbvReorderBurstCtx>(burst->custom_pkt_data);
  if (ctx && !ctx->released && ctx->plan != nullptr &&
      ctx->out_idx < ctx->plan->out_bufs.size()) {
...
void IbverbsMgr::reorder_cleanup(IbvRxQueue& q) {
  if (!q.reorder) { return; }
  for (auto& plan : q.reorder->plans) {
    for (auto& ob : plan.out_bufs) {
      if (ob.event) { cudaEventDestroy(ob.event); }
      if (ob.h_batch_id) { cudaFreeHost(ob.h_batch_id); }
      if (ob.d_batch_id) { cudaFree(ob.d_batch_id); }
    }
    if (plan.d_input_ptrs) { cudaFree(plan.d_input_ptrs); }
  }
  q.reorder.reset();
}

Status IbverbsMgr::set_reorder_cuda_stream(const std::string& interface_name,
                                           const std::string& reorder_name, cudaStream_t stream) {
  const int port = get_port_id(interface_name);
  for (auto& q : rx_queues_) {
    if (q->port_id != port || !q->reorder) { continue; }
    for (auto& plan : q->reorder->plans) {
...
Status IbverbsMgr::get_reorder_burst_info(BurstParams* burst, ReorderBurstInfo* info) {
  ...
  if (ctx->h_batch_id != nullptr) {
    if (burst->event != nullptr) {
      ...
      if (s == cudaErrorNotReady) { return Status::NOT_READY; }
      if (s != cudaSuccess) {
...
Status IbverbsMgr::get_rx_burst(BurstParams** burst, int port, int q) {
  IbvRxQueue* rq = find_rx_queue(port, q);
  if (rq == nullptr) { return Status::INVALID_PARAMETER; }
  if (rq->reorder && rq->reorder->enabled) { return reorder_get_rx(*rq, burst); }
  void* b = nullptr;
  if (rte_ring_dequeue(rq->ring, &b) != 0) { return Status::NOT_READY; }
...
void IbverbsMgr::free_packet(BurstParams* burst, int pkt) {
...
void IbverbsMgr::free_all_packets(BurstParams* burst) {
  if (burst == nullptr) { return; }
  if (burst->hdr.hdr.burst_flags & DAQIRI_BURST_FLAG_REORDERED) {
  ...
  for (int i = 0; i < num; i++) { release_strides(*q, wqe_arr[i], strd_arr[i]); }
}

void IbverbsMgr::free_packet_segment(BurstParams* burst, int seg, int pkt) {
  // Segments alias one stride; releasing seg 0 releases the packet.
  if (seg == 0) { free_packet(burst, pkt); }
}

void IbverbsMgr::free_all_segment_packets(BurstParams* burst, int seg) {
  if (seg == 0) { free_all_packets(burst); }
}

void IbverbsMgr::free_rx_burst(BurstParams* burst) {
  if (burst == nullptr) { return; }
  if (burst->hdr.hdr.burst_flags & DAQIRI_BURST_FLAG_REORDERED) {
...
void IbverbsMgr::free_rx_metadata(BurstParams* burst) {
  if (burst == nullptr) { return; }
  if (burst->hdr.hdr.burst_flags & DAQIRI_BURST_FLAG_REORDERED) {
    reorder_release_output(burst);
    return;
...
// Accessors
// ---------------------------------------------------------------------------
void* IbverbsMgr::get_packet_ptr(BurstParams* burst, int idx) { return burst->pkts[0][idx]; }

uint32_t IbverbsMgr::get_packet_length(BurstParams* burst, int idx) {
  return burst->pkt_lens[0][idx];
}

void* IbverbsMgr::get_segment_packet_ptr(BurstParams* burst, int seg, int idx) {
  return burst->pkts[seg][idx];
}

uint32_t IbverbsMgr::get_segment_packet_length(BurstParams* burst, int seg, int idx) {
  return burst->pkt_lens[seg][idx];
}

uint16_t IbverbsMgr::get_packet_flow_id(BurstParams* burst, int idx) {
  ...
  if (tag != 0) { return static_cast<uint16_t>(tag); }
  IbvRxQueue* q = find_rx_queue(burst->hdr.hdr.port_id, burst->hdr.hdr.q_id);
...
uint64_t IbverbsMgr::ts_to_ns(struct ibv_context* ctx, uint64_t raw_ts) {
...
Status IbverbsMgr::get_packet_rx_timestamp(BurstParams* burst, int idx, uint64_t* timestamp_ns) {
  ...
  *timestamp_ns = 0;
  ...
  if (burst->hdr.hdr.burst_flags & DAQIRI_BURST_FLAG_REORDERED) { return Status::NOT_SUPPORTED; }
  IbvRxQueue* q = find_rx_queue(burst->hdr.hdr.port_id, burst->hdr.hdr.q_id);
  if (q == nullptr) { return Status::INVALID_PARAMETER; }
  *timestamp_ns = ts_to_ns(q->ctx, burst_ts_arr(burst)[idx]);
  return Status::SUCCESS;
}

void* IbverbsMgr::get_packet_extra_info(BurstParams* burst, int idx) {
  (void)burst;
  (void)idx;
  return nullptr;
}

uint64_t IbverbsMgr::get_burst_tot_byte(BurstParams* burst) {
  uint64_t tot = 0;
  for (size_t i = 0; i < burst->hdr.hdr.num_pkts; i++) { tot += burst->pkt_lens[0][i]; }
  return tot;
}

uint16_t IbverbsMgr::get_num_rx_queues(int port_id) const {
  uint16_t n = 0;
  for (const auto& q : rx_queues_) {
    if (q->port_id == port_id) { ++n; }
  }
  return n;
}

std::string IbverbsMgr::port_netdev(int port) const {
  // Resolve port -> ibv device -> netdev. The RDMA device exposes its netdev
  // under /sys/class/infiniband/<dev>/device/net/<netdev>.
  struct ibv_context* ctx = nullptr;
  for (const auto& q : rx_queues_) {
    if (q->port_id == port) { ctx = q->ctx; break; }
  }
  if (ctx == nullptr) {
...
void IbverbsMgr::ensure_port_mtus() {
  ...
  for (const auto& intf : cfg_.ifs_) {
    ...
    auto accumulate = [&](const CommonQueueConfig& c) {
      ...
      for (const auto& name : c.mrs_) {
        ...
        if (it != cfg_.mrs_.end()) { sum += it->second.buf_size_; }
      }
      max_frame = std::max(max_frame, sum);
    };
    for (const auto& q : intf.rx_.queues_) { accumulate(q.common_); }
    for (const auto& q : intf.tx_.queues_) { accumulate(q.common_); }
    if (max_frame <= kStdMtu + kEthHdr) { continue; }  // standard MTU already fits
...
Status IbverbsMgr::get_mac_addr(int port, char* mac) {
  ...
  for (int i = 0; i < 6; i++) { mac[i] = static_cast<char>(b[i]); }
  return Status::SUCCESS;
}

bool IbverbsMgr::validate_config() const { return true; }

void IbverbsMgr::print_stats() {
  for (auto& q : rx_queues_) {
    DAQIRI_LOG_INFO(
...
void IbverbsMgr::shutdown() {
  ...
  for (auto& q : tx_queues_) {
    q->running = false;
    if (q->compl_worker.joinable()) { q->compl_worker.join(); }
  }
  ...
  for (auto& [port, st] : port_steering_) {
    for (auto* r : st.rules) {
      if (r) { mlx5dv_dr_rule_destroy(r); }
    }
    for (auto* a : st.tag_actions) {
      if (a) { mlx5dv_dr_action_destroy(a); }
    }
    for (auto* m : st.matchers) {
      if (m) { mlx5dv_dr_matcher_destroy(m); }
    }
    if (st.table) { mlx5dv_dr_table_destroy(st.table); }
    if (st.domain) { mlx5dv_dr_domain_destroy(st.domain); }
    // Flex-parser nodes referenced by misc4 matchers -- destroy after the rules.
    for (auto& [id, fn] : st.flex_nodes) {
      if (fn.obj) { mlx5dv_devx_obj_destroy(fn.obj); }
    }
    if (st.ipv4_len_node.obj) { mlx5dv_devx_obj_destroy(st.ipv4_len_node.obj); }
  }
  ...
  for (auto& q : rx_queues_) {
    ...
    if (q->ind_table) { ibv_destroy_rwq_ind_table(q->ind_table); }
    if (q->wq) { ibv_destroy_wq(q->wq); }
    if (q->cq) { ibv_destroy_cq(q->cq); }
    if (q->ring) { rte_ring_free(q->ring); }
  }
  rx_queues_.clear();
  for (auto& q : tx_queues_) {
    if (q->qp) { ibv_destroy_qp(q->qp); }
    if (q->cq) { ibv_destroy_cq(q->cq); }
    if (q->send_ring) { rte_ring_free(q->send_ring); }
  }
  tx_queues_.clear();
  for (auto& pd : pd_map_) {
    if (pd.second) { ibv_dealloc_pd(pd.second); }
  }
  pd_map_.clear();
  for (auto& c : ctx_map_) {
    if (c.second) { ibv_close_device(c.second); }
  }
  ctx_map_.clear();
  if (rx_meta_pool_) {
...
// ---------------------------------------------------------------------------
IbvTxQueue* IbverbsMgr::find_tx_queue(int port, int q) {
  for (auto& tq : tx_queues_) {
    if (tq->port_id == port && tq->queue_id == q) { return tq.get(); }
  }
  return nullptr;
}

Status IbverbsMgr::create_tx_raw_qp(IbvTxQueue& q) {
...
Status IbverbsMgr::setup_tx_queue(IbvTxQueue& q, const InterfaceConfig& intf,
  ...
  for (const auto& off : qcfg.common_.offloads_) {
    if (off == "tx_eth_src") { q.insert_eth_src = true; }
  }
  if (qcfg.common_.mrs_.empty()) {
  ...
  if (Status s = create_tx_raw_qp(q); s != Status::SUCCESS) { return s; }
  ...
  q.send_ring = rte_ring_create(send_name.c_str(), 4096, rte_socket_id(),
...
void IbverbsMgr::poll_tx_completions(IbvTxQueue& q) {
  ...
  for (;;) {
    ...
    if (opcode == MLX5_CQE_INVALID || owner != phase) { break; }
    cqe_read_barrier();
...
void IbverbsMgr::tx_worker(std::vector<IbvTxQueue*> group) {
  if (group.empty()) { return; }
...
BurstParams* IbverbsMgr::create_tx_burst_params() {
  BurstParams* burst = nullptr;
  if (rte_mempool_get(tx_meta_pool_, reinterpret_cast<void**>(&burst)) < 0) { return nullptr; }
  burst->hdr.hdr.num_pkts = 0;
  burst->hdr.hdr.num_segs = 1;
  burst->hdr.hdr.burst_flags = IBV_TX_BURST_FLAG;
  burst->pkts[0] =
      reinterpret_cast<void**>(reinterpret_cast<uint8_t*>(burst) + g_layout.off_pkts0);
  burst->pkt_lens[0] =
      reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(burst) + g_layout.off_lens0);
  burst->pkts[1] =
      reinterpret_cast<void**>(reinterpret_cast<uint8_t*>(burst) + g_layout.off_pkts1);
  burst->pkt_lens[1] =
      reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(burst) + g_layout.off_lens1);
  return burst;
}

Status IbverbsMgr::get_tx_metadata_buffer(BurstParams** burst) {
  *burst = create_tx_burst_params();
  return *burst ? Status::SUCCESS : Status::NO_FREE_BURST_BUFFERS;
}

bool IbverbsMgr::is_tx_burst_available(BurstParams* burst) {
...
Status IbverbsMgr::get_tx_packet_burst(BurstParams* burst) {
  IbvTxQueue* q = find_tx_queue(burst->hdr.hdr.port_id, burst->hdr.hdr.q_id);
  if (q == nullptr) { return Status::INVALID_PARAMETER; }
  ...
  const uint64_t in_flight = q->alloc_head - q->completed_tail.load(std::memory_order_acquire);
  if (q->num_slots - in_flight < n) { return Status::NO_FREE_PACKET_BUFFERS; }
  ...
  if (q->send_scheduling) { memset(burst_ts_arr(burst), 0, n * sizeof(uint64_t)); }
  return Status::SUCCESS;
}

Status IbverbsMgr::set_packet_lengths(BurstParams* burst, int idx,
                                      const std::initializer_list<int>& lens) {
  int seg = 0;
  for (int v : lens) {
    if (seg >= MAX_NUM_SEGS) { break; }
...
Status IbverbsMgr::set_eth_header(BurstParams* burst, int idx, char* dst_addr) {
  ...
  IbvTxQueue* q = find_tx_queue(burst->hdr.hdr.port_id, burst->hdr.hdr.q_id);
  if (q != nullptr && q->insert_eth_src) { memcpy(pkt->eth.h_source, q->eth_src, 6); }
  return Status::SUCCESS;
}

Status IbverbsMgr::set_ipv4_header(BurstParams* burst, int idx, int ip_len, uint8_t proto,
...
Status IbverbsMgr::set_udp_header(BurstParams* burst, int idx, int udp_len, uint16_t src_port,
                                  uint16_t dst_port) {
  auto* pkt = static_cast<UDPIPV4Pkt*>(burst->pkts[0][idx]);
...
Status IbverbsMgr::set_udp_payload(BurstParams* burst, int idx, void* data, int len) {
...
Status IbverbsMgr::set_packet_tx_time(BurstParams* burst, int idx, uint64_t time) {
  IbvTxQueue* q = find_tx_queue(burst->hdr.hdr.port_id, burst->hdr.hdr.q_id);
  if (q == nullptr) { return Status::INVALID_PARAMETER; }
  if (!q->send_scheduling) { return Status::NOT_SUPPORTED; }
...
void IbverbsMgr::post_tx_burst(IbvTxQueue& q, BurstParams* burst) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (q.send_scheduling && txtime[i] != 0) {
      ...
      wctrl->opmod_idx_opcode =
          htobe32((static_cast<uint32_t>(MLX5_OPC_MOD_WAIT_TIME) << 24) |
...
Status IbverbsMgr::send_tx_burst(BurstParams* burst) {
  IbvTxQueue* q = find_tx_queue(burst->hdr.hdr.port_id, burst->hdr.hdr.q_id);
  if (q == nullptr) { return Status::INVALID_PARAMETER; }
...
void IbverbsMgr::free_tx_burst(BurstParams* burst) {
  if (burst != nullptr) { rte_mempool_put(tx_meta_pool_, burst); }
}

void IbverbsMgr::free_tx_metadata(BurstParams* burst) {
  if (burst != nullptr) { rte_mempool_put(tx_meta_pool_, burst); }
}
...
// ---------------------------------------------------------------------------
Status IbverbsMgr::drop_all_traffic(int port) {
  ...
  if (it == port_steering_.end()) { return Status::SUCCESS; }
...
Status IbverbsMgr::allow_all_traffic(int port) {
  auto it = port_steering_.find(port);
  if (it == port_steering_.end()) { return Status::SUCCESS; }
  PortSteering& st = it->second;
  if (!st.dropped) { return Status::SUCCESS; }
  for (auto& spec : st.rule_specs) {
    ...
    struct mlx5dv_dr_action* actions[2];
    int na = 0;
    if (spec.tag != nullptr) { actions[na++] = spec.tag; }
