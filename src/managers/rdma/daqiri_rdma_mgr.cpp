...
uint32_t RdmaMgr::get_segment_packet_length(BurstParams *burst, int seg,
                                            int idx) {
...
Status RdmaMgr::set_ipv4_header(BurstParams *burst, int idx, int ip_len,
...
Status RdmaMgr::set_udp_header(BurstParams *burst, int idx, int udp_len,
                               uint16_t src_port, uint16_t dst_port) {
  DAQIRI_LOG_CRITICAL("Cannot set UDP header in RDMA mode");
  return Status::NOT_SUPPORTED;
}

Status RdmaMgr::set_udp_payload(BurstParams *burst, int idx, void *data,
                                int len) {
  rte_memcpy(burst->pkts[0][idx], data, len);
  return Status::SUCCESS;
}

uint64_t RdmaMgr::get_burst_tot_byte(BurstParams *burst) { return 0; }

BurstParams *RdmaMgr::create_tx_burst_params() { return new BurstParams(); }

bool RdmaMgr::get_ip_from_interface(const std::string_view &if_name,
                                    sockaddr_in &addr) {
...
inline bool RdmaMgr::ack_event(rdma_cm_event *cm_event) {
  ...
  if (ret != 0) {
    ...
    DAQIRI_LOG_INFO("Acknowledged CM event: {}",
...
int RdmaMgr::rdma_register_mr(const MemoryRegionConfig &mr, void *ptr) {
  ...
  params.params_ = mr;
  params.ptr_ = ptr;
  DAQIRI_LOG_INFO("Registering MR {} with ibverbs {}", mr.name_,
                  pd_map_.size());
  ...
  for (const auto &pd : pd_map_) {
    if (pd.second != nullptr) {
      ...
      if (params.ctx_mr_map_[pd.second] == nullptr) {
        DAQIRI_LOG_CRITICAL("Failed to register MR {} on PD {}", mr.name_,
                            (void *)pd.second);
        return -1;
      } else {
        DAQIRI_LOG_INFO("Successfully registered MR {} with {} bytes on PD {} "
                        "ptr {}-{} lkey {} access {}",
                        mr.name_, mr.buf_size_ * mr.num_bufs_,
                        (void *)pd.second, (void *)ptr,
                        (void *)((uint8_t *)ptr + mr.adj_size_ * mr.num_bufs_),
                        params.ctx_mr_map_[pd.second]->lkey, mr.access_);
      }
    }
  }
...
int RdmaMgr::rdma_register_cfg_mrs() {
  ...
  for (const auto &mr : cfg_.mrs_) {
    ...
    auto ring = rte_ring_create(mr.second.name_.c_str(),
                                rte_align32pow2(mr.second.num_bufs_ + 1),
                                rte_socket_id(), 0);
...
Status RdmaMgr::wait_on_key_xchg() { return Status::SUCCESS; }
...
int RdmaMgr::setup_thread_params(rdma_thread_params *params, bool is_server) {
  ...
  DAQIRI_LOG_INFO("Creating RX and TX CQs for client ID {}",
                  (void *)params->client_id);
  qp_params.rx_cq =
      ibv_create_cq(params->client_id->verbs, MAX_CQ, nullptr, nullptr, 0);
  ...
  if (qp_params.tx_cq == nullptr) {
    ...
    DAQIRI_LOG_INFO("Successfully created TX CQ for client ID {}",
                    (void *)params->client_id);
  }

  memset(&qp_params.qp_attr, 0, sizeof(qp_params.qp_attr));
  qp_params.qp_attr.cap.max_recv_sge =
      1; // No header-data split in RDMA right now
  ...
  if (cfg_.ifs_[static_cast<int>(params->if_idx)].rdma_.xmode_ ==
      RDMATransportMode::RC) {
    qp_params.qp_attr.qp_type = IBV_QPT_RC;
  } else if (cfg_.ifs_[static_cast<int>(params->if_idx)].rdma_.xmode_ ==
             RDMATransportMode::UC) {
    qp_params.qp_attr.qp_type = IBV_QPT_UC;
  } else if (cfg_.ifs_[static_cast<int>(params->if_idx)].rdma_.xmode_ ==
             RDMATransportMode::UD) {
    qp_params.qp_attr.qp_type = IBV_QPT_UD;
  } else {
    DAQIRI_LOG_ERROR(
        "RDMA transport mode {} not supported!",
        static_cast<int>(
            cfg_.ifs_[static_cast<int>(params->if_idx)].rdma_.xmode_));
    return -1;
  }
  ...
  DAQIRI_LOG_INFO("Creating QP for client ID {} with PD {}",
                  (void *)params->client_id, (void *)params->pd);
  ...
  DAQIRI_LOG_INFO("Successfully configured queues for client {}",
                  (void *)params->client_id);
...
int RdmaMgr::destroy_thread_params(rdma_thread_params *params) {
  DAQIRI_LOG_INFO("Destroying thread params for client {}",
                  (void *)params->client_id);
...
Status RdmaMgr::send_tx_burst(BurstParams *burst) {
  ...
  auto ri = tx_rings_map_.find(
      reinterpret_cast<struct rdma_cm_id *>(burst->rdma_hdr.conn_id));
...
void RdmaMgr::rdma_thread(bool is_server, rdma_thread_params *tparams) {
  ...
  DAQIRI_LOG_INFO("Affined {} RDMA thread to core {}",
                  is_server ? "Server" : "Client", cpu_core);
  ...
  while (!rdma_force_quit.load()) {
    ...
    while ((num_comp = ibv_poll_cq(tparams->qp_params.rx_cq, 1, &wc)) != 0) {
      DAQIRI_LOG_DEBUG("GOT RX COMPLETION in thread {} core {} wrid {}",
                       (void *)tparams->client_id, cpu_core, (int64_t)wc.wr_id);
      if (wc.status != IBV_WC_SUCCESS) {
        DAQIRI_LOG_ERROR("CQ error {} for WRID {} and opcode {}",
                         (int)wc.status, (int64_t)wc.wr_id, (int)wc.opcode);
        continue;
      }
      ...
      if (wc.opcode == IBV_WC_RECV) {
        ...
        if (it == outstanding_receive_wr_ids.end()) {
          DAQIRI_LOG_CRITICAL(
              "WR ID {} not found in outstanding RECEIVE WR IDs", wc.wr_id);
          continue;
        }

        msg = it->second;

        if (msg->rdma_hdr.conn_id !=
            reinterpret_cast<uintptr_t>(tparams->client_id)) {
          DAQIRI_LOG_CRITICAL(
              "Wrong connection ID in receive completion {}: {} != {}",
              wc.wr_id, msg->rdma_hdr.conn_id,
              reinterpret_cast<uintptr_t>(tparams->client_id));
        }
      ...
      msg->rdma_hdr.status = wc.status == IBV_WC_SUCCESS
                                 ? Status::SUCCESS
    ...
    while ((num_comp = ibv_poll_cq(tparams->qp_params.tx_cq, 1, &wc)) != 0) {
      ...
      if (wc.opcode == IBV_WC_SEND) {
        ...
        if (it == outstanding_send_wr_ids.end()) {
          DAQIRI_LOG_CRITICAL("WR ID {} not found in outstanding SEND WR IDs",
                              wc.wr_id);
          continue;
        }
        ...
        if (msg->rdma_hdr.conn_id !=
            reinterpret_cast<uintptr_t>(tparams->client_id)) {
          DAQIRI_LOG_CRITICAL(
              "Wrong connection ID in send completion {}: {} != {}", wc.wr_id,
    ...
    // nullptr);
    if (rte_ring_dequeue(tparams->qp_params.tx_ring,
    ...
    case RDMAOpCode::SEND: {
      ...
      for (int p = 0; p < burst->rdma_hdr.num_pkts; p++) {
        ...
        wr.wr_id =
            burst->rdma_hdr.wr_id + p; // Auto-increment wr_id to be unique
        wr.sg_list = &sge;
        wr.num_sge = 1;
        ...
        if (ret != 0) {
          DAQIRI_LOG_CRITICAL("Failed to post SEND request, errno: {}",
                              strerror(errno));
          free_tx_burst(burst);
          continue;
        }
  ...
  DAQIRI_LOG_INFO("{} RDMA thread exiting on core {}",
                  is_server ? "Server" : "Client", cpu_core);
}

Status RdmaMgr::rdma_connect_to_server(const std::string &dst_addr,
...
Status RdmaMgr::rdma_set_header(BurstParams *burst, RDMAOpCode op_code,
  ...
  snprintf(burst->rdma_hdr.local_mr_name, sizeof(burst->rdma_hdr.local_mr_name),
           "%s", local_mr_name.c_str());
  return Status::SUCCESS;
}

Status RdmaMgr::rdma_get_server_conn_id(const std::string &server_addr,
                                        uint16_t server_port,
                                        uintptr_t *conn_id) {
  const auto iter =
      server_str_to_id_.find(server_addr + ":" + std::to_string(server_port));
  if (iter == server_str_to_id_.end()) {
    DAQIRI_LOG_CRITICAL("Couldn't find server params for address {}",
                        server_addr);
    return Status::INVALID_PARAMETER;
  }
  ...
  for (size_t i = 0; i < server_params->second.size(); i++) {
    if (server_params->second[i].client_id != nullptr &&
        !server_params->second[i].active) {
      *conn_id =
          reinterpret_cast<uintptr_t>(server_params->second[i].client_id);
  ...
  DAQIRI_LOG_CRITICAL("Couldn't find an available queue ID for server {}:{}. "
                      "Maybe no clients have tried to connect yet?",
                      server_addr, server_port);
  return Status::NO_SPACE_AVAILABLE;
}
...
Status RdmaMgr::rdma_connect_to_server(const std::string &dst_addr,
  ...
  if (!initialized_) {
    DAQIRI_LOG_WARN(
        "RDMA manager not initialized yet. Not trying to connect to server");
  ...
  *conn_id = 0;
  ...
  struct sockaddr *src_addr_p =
      src_addr.empty() ? nullptr : (struct sockaddr *)&src_addr_in;
  ...
  if (event->event != RDMA_CM_EVENT_ADDR_RESOLVED) {
    if (event->event == RDMA_CM_EVENT_ADDR_ERROR) {
      ...
      DAQIRI_LOG_CRITICAL("Unexpected event from rdma_resolve_addr: {}",
                          (int)event->event);
    }
    rdma_destroy_event_channel(ec);
    return Status::CONNECT_FAILURE;
  ...
  if (event->event != RDMA_CM_EVENT_ROUTE_RESOLVED) {
    if (event->event == RDMA_CM_EVENT_REJECTED) {
      DAQIRI_LOG_WARN("Server rejected connection. Check if server is running "
  ...
  inet_ntop(AF_INET, &source_addr->sa_data[2], source_addr_str,
            INET_ADDRSTRLEN);
  ...
  if (client_port < 0) {
    DAQIRI_LOG_ERROR("No configured interface matches source address {}",
                     source_addr_str);
  ...
  if (event->event != RDMA_CM_EVENT_ESTABLISHED) {
    ...
    DAQIRI_LOG_INFO("Client {} established connection to server",
                    (void *)cm_id);
  }
  ...
  threads_mutex_.lock();
  worker_threads_[cm_id] =
      std::thread(&RdmaMgr::rdma_thread, this, false, &params);
  threads_mutex_.unlock();

  *conn_id = reinterpret_cast<uintptr_t>(cm_id);
  DAQIRI_LOG_INFO("Successfully connected to server {} on port {}", dst_addr,
                  dst_port);
...
Status RdmaMgr::rdma_get_port_queue(uintptr_t conn_id, uint16_t *port,
                                    uint16_t *queue) {
  // Look up connection ID to get port/q
  auto iter =
      client_q_params_.find(reinterpret_cast<struct rdma_cm_id *>(conn_id));
  if (iter == client_q_params_.end()) {
    DAQIRI_LOG_CRITICAL("Failed to find client params for connection ID {}",
                        conn_id);
    return Status::INVALID_PARAMETER;
  }
...
Status RdmaMgr::get_tx_packet_burst(BurstParams *burst) {
  ...
  if (rte_mempool_get(tx_burst_pool_,
                      reinterpret_cast<void **>(&burst->pkts[0])) != 0) {
  ...
  int rx = rte_ring_dequeue_bulk(burst_pool->second,
                                 ...
                                 burst->rdma_hdr.num_pkts, nullptr);
  if (rx != burst->rdma_hdr.num_pkts) {
    DAQIRI_LOG_ERROR("Asked for {} packets, got {}", burst->rdma_hdr.num_pkts,
                     rx);
  ...
  if (rte_mempool_get(pkt_len_pool_,
                      reinterpret_cast<void **>(&burst->pkt_lens[0])) != 0) {
...
void RdmaMgr::run() {
  ...
  for (const auto &intf : cfg_.ifs_) {
    ...
    if (ret != 0) {
      DAQIRI_LOG_CRITICAL("Failed to create RDMA server ID {}",
                          strerror(errno));
      return;
    }
    ...
    DAQIRI_LOG_INFO(
        "Created RDMA server on {}:{} successfully with listener_id {}",
        intf.address_, intf.rdma_.port_, (void *)s_id);

    ret =
        rdma_bind_addr(s_id, reinterpret_cast<struct sockaddr *>(&server_addr));
    if (ret != 0) {
      DAQIRI_LOG_CRITICAL("Failed to bind for RDMA server: {}",
                          strerror(errno));
      return;
    }
    ...
    if (ret != 0) {
      DAQIRI_LOG_CRITICAL("Failed to listen for RDMA server: {}",
                          strerror(errno));
      return;
    }

    server_str_to_id_[intf.address_ + ":" + std::to_string(intf.rdma_.port_)] =
        s_id;

    DAQIRI_LOG_INFO("RDMA server successfully started on {} queues",
  ...
  while (!rdma_force_quit.load()) {
    ...
    if (cm_event->status != 0) {
      ...
      if (rdma_ack_cm_event(cm_event) == 0) {
        ...
        DAQIRI_LOG_CRITICAL("Failed to acknowledge CM event {}: {}",
                            rdma_event_str(cm_event->event), strerror(errno));
      }
    ...
    case RDMA_CM_EVENT_CONNECT_REQUEST: {
      DAQIRI_LOG_INFO(
          "Received new connection request for client ID {} on listener {}",
      ...
      if (listen_iter == pd_params_.end()) {
        DAQIRI_LOG_ERROR("Failed to find server ID for {}",
        ...
        if (queue_idx == -1) {
          DAQIRI_LOG_CRITICAL("No free queues on server ID {}. Close at least "
        ...
        if (rdma_accept(params.client_id, &conn_param) != 0) {
          DAQIRI_LOG_CRITICAL("Failed to accept connection: {}",
                              strerror(errno));
    ...
    case RDMA_CM_EVENT_ESTABLISHED: {
      bool found = false;
      DAQIRI_LOG_INFO("Received established event for client ID {}",
                      (void *)cm_event->id);
      ...
      for (auto &sp : server_q_params_) {
        for (auto &thread_params : sp.second) {
          if (thread_params.client_id == cm_event->id) {
            DAQIRI_LOG_INFO("Client ID {} is on server ID {}",
                            (void *)cm_event->id, (void *)sp.first);
            found = true;

            DAQIRI_LOG_INFO(
                "Connection established. Launching server thread for client {}",
                (void *)cm_event->id);
      ...
      if (!found) {
        DAQIRI_LOG_CRITICAL(
            "Received established event for unknown client ID {}",
            (void *)cm_event->id);
        break;
      }
    ...
    case RDMA_CM_EVENT_DISCONNECTED: {
      DAQIRI_LOG_INFO("Received disconnected event for client ID {}",
                      (void *)cm_event->id);
      ...
      for (auto &sp : server_q_params_) {
        for (auto &thread_params : sp.second) {
          if (thread_params.client_id == cm_event->id) {
            ...
            DAQIRI_LOG_INFO("Joined and removed client thread for ID {}",
                            (void *)cm_event->id);
            found = true;
            break;
          }
      ...
      if (!found) {
        DAQIRI_LOG_CRITICAL(
            "Received disconnected event for unknown client ID {}",
            (void *)cm_event->id);
      }
    ...
    default: {
      DAQIRI_LOG_INFO("Cannot handle event type {}",
                      rdma_event_str(cm_event->event));
      ack_event(cm_event);
      break;
    }
...
int RdmaMgr::setup_pools_and_rings() {
  ...
  pkt_len_pool_ = rte_mempool_create(
      "PKT_LEN_POOL", (1U << 7) - 1U, sizeof(uint32_t) * MAX_RDMA_BATCH, 0, 0,
  ...
  DAQIRI_LOG_DEBUG("Setting up RX meta pool");
  rx_meta = rte_mempool_create("RX_META_POOL", (1U << 6) - 1U,
  ...
  tx_meta = rte_mempool_create("TX_META_POOL", (1U << 6) - 1U,
                               sizeof(BurstParams), 0, 0, nullptr, nullptr,
                               nullptr, nullptr, rte_socket_id(), 0);
  if (tx_meta == nullptr) {
  ...
  tx_burst_pool_ = rte_mempool_create(
      "TX_BURST_POOL", (1U << 7) - 1U, sizeof(void *) * MAX_RDMA_BATCH, 0, 0,
      nullptr, nullptr, nullptr, nullptr, rte_socket_id(), 0);
...
void RdmaMgr::free_tx_burst(BurstParams *burst) {
  ...
  if (burst_pool != mem_pools_.end()) {
    ...
    if (ret != burst->rdma_hdr.num_pkts) {
      DAQIRI_LOG_CRITICAL("Asked to free {} packets, only enqueued {}",
                          burst->rdma_hdr.num_pkts, ret);
    }
  }
...
void RdmaMgr::initialize() {
  ...
  for (int i = 0; i < num_ib_devices; i++) {
    ...
    if (ibv_query_device(device, &device_attr) != 0) {
      DAQIRI_LOG_ERROR("Failed to query device attributes for device {}",
                       (void *)device);
      continue;
    }
  ...
  for (auto &mr : cfg_.mrs_) {
    mr.second.adj_size_ =
        RTE_ALIGN_CEIL(mr.second.buf_size_, get_alignment(mr.second.kind_));
  }
...
Status RdmaMgr::get_rx_burst(BurstParams **burst, uintptr_t conn_id,
                             bool server) {
  if (server) {
    const auto ring =
        rx_rings_map_[reinterpret_cast<struct rdma_cm_id *>(conn_id)];
    if (ring == nullptr) {
...
void RdmaMgr::print_stats() {
  ...
  for (const auto &it : tx_rings_map_) {
    if (it.second != nullptr) {
      tx_ring_entries += rte_ring_count(it.second);
    }
  }
  for (const auto &it : rx_rings_map_) {
    if (it.second != nullptr) {
      rx_ring_entries += rte_ring_count(it.second);
    }
  }
  ...
  DAQIRI_LOG_INFO("  initialized={} stop_requested={}", initialized_,
                  rdma_force_quit.load() ? "true" : "false");
  DAQIRI_LOG_INFO(
      "  listeners={} server_queues_total={} server_queues_with_client={} "
      "server_queues_active={}",
      server_q_params_.size(), total_server_queues, server_queues_with_client,
      active_server_queues);
  DAQIRI_LOG_INFO("  client_connections={} active_client_connections={}",
                  client_q_params_.size(), active_client_queues);
  DAQIRI_LOG_INFO(
      "  worker_threads={} pds={} registered_mrs={} remote_mrs={} endpoints={}",
      worker_threads, pd_map_.size(), mrs_.size(), remote_mrs_.size(),
      endpoints_.size());
  DAQIRI_LOG_INFO(
      "  tx_rings={} tx_ring_pending={} rx_rings={} rx_ring_pending={}",
      tx_rings_map_.size(), tx_ring_entries, rx_rings_map_.size(),
      rx_ring_entries);
...
Status RdmaMgr::register_mr(std::string name, int intf, void *addr, size_t len,
                            int flags) {
