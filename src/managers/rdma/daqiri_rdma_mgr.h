...
public:
  ...
  Status register_mr(std::string name, int intf, void *addr, size_t len,
                     int flags);
...
private:
  ...
  static constexpr int MAX_NUM_MR =
      16; // Maximum number of memory registers to exchange
  ...
  std::unordered_map<struct rdma_cm_id *, std::vector<rdma_thread_params>>
      server_q_params_;
  ...
  std::unordered_map<struct rdma_cm_id *,
                     std::unordered_map<std::string, rdma_remote_mr_info>>
      endpoints_;
  ...
  bool get_ip_from_interface(const std::string_view &if_name,
                             sockaddr_in &addr);
