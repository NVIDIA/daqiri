...
public:
  ...
  Status set_packet_tx_time(BurstParams *burst, int idx,
  ...
  Status get_rx_burst(BurstParams **burst, uintptr_t conn_id,
  ...
  Status socket_connect_to_server(const std::string &dst_addr,
                                  uint16_t dst_port,
  ...
  Status rdma_connect_to_server(const std::string &dst_addr, uint16_t dst_port,
                                const std::string &src_addr,
                                uintptr_t *conn_id) override;
  Status rdma_get_port_queue(uintptr_t conn_id, uint16_t *port,
                             uint16_t *queue) override;
...
private:
  ...
  std::shared_ptr<RxQueueState> get_or_create_rx_queue(uint16_t port,
                                                       uint16_t queue);

  std::shared_ptr<ConnectionState>
  create_tcp_client_connection(EndpointState &ep, const std::string &dst_addr,
                               uint16_t dst_port, const std::string &src_addr,
                               uint16_t src_port, bool set_as_primary);

  std::shared_ptr<ConnectionState>
  register_connection(int fd, uint16_t port, uint16_t queue, int if_index,
                      bool server_side, bool is_udp,
  ...
  Status pop_rx_burst(const std::shared_ptr<RxQueueState> &qstate,
                      BurstParams **burst);
  void push_rx_burst(const std::shared_ptr<RxQueueState> &qstate,
                     BurstParams *burst);
  ...
  bool send_tcp_burst(int fd, BurstParams *burst, size_t *sent_pkts,
                      uint64_t *sent_bytes);
