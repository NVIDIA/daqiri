...
Status SocketMgr::roce_not_initialized(const char *op_name) const {
  DAQIRI_LOG_ERROR(
      "{} is only supported when protocol=roce and RoCE manager is initialized",
      op_name);
...
bool SocketMgr::set_config_and_initialize(const NetworkConfig &cfg) {
  ...
  if (is_roce_protocol()) {
    ...
    return initialized_;
#else
    DAQIRI_LOG_ERROR("Socket backend built without RDMA support; protocol=roce "
                     "is unavailable");
    initialized_ = false;
    return false;
#endif
...
void SocketMgr::initialize() {
  ...
  try {
    ...
    for (size_t i = 0; i < cfg_.ifs_.size(); ++i) {
      ...
      ep->max_packet_size =
          static_cast<size_t>(std::max(1, select_max_packet_size(if_cfg)));
    ...
    for (auto &ep : endpoints_) {
      if (cfg_.common_.protocol == SocketProtocol::TCP) {
        ...
        DAQIRI_LOG_ERROR("Unsupported socket protocol '{}'",
    ...
    initialized_ = true;
    DAQIRI_LOG_INFO("Socket manager initialized for protocol '{}'",
                    socket_protocol_to_string(cfg_.common_.protocol));
...
void *SocketMgr::get_segment_packet_ptr(BurstParams *burst, int seg, int idx) {
  if (burst == nullptr || seg < 0 || seg >= MAX_NUM_SEGS ||
      burst->pkts[seg] == nullptr || idx < 0 ||
...
uint32_t SocketMgr::get_segment_packet_length(BurstParams *burst, int seg,
                                              int idx) {
  if (burst == nullptr || seg < 0 || seg >= MAX_NUM_SEGS ||
      burst->pkt_lens[seg] == nullptr || idx < 0 ||
      idx >= static_cast<int>(burst->hdr.hdr.num_pkts)) {
    return 0;
  }
...
Status SocketMgr::get_tx_packet_burst(BurstParams *burst) {
  if (is_roce_protocol()) {
#if DAQIRI_MGR_RDMA
    return roce_mgr_ != nullptr ? roce_mgr_->get_tx_packet_burst(burst)
                                : roce_not_initialized("get_tx_packet_burst");
#else
  ...
  const auto num_pkts = burst->hdr.hdr.num_pkts > 0
                            ? burst->hdr.hdr.num_pkts
                            : (ep ? ep->tx_batch_size : 1);
...
Status SocketMgr::set_eth_header(BurstParams *burst, int idx, char *dst_addr) {
  if (is_roce_protocol()) {
#if DAQIRI_MGR_RDMA
    return roce_mgr_ != nullptr
               ? roce_mgr_->set_eth_header(burst, idx, dst_addr)
  ...
  DAQIRI_LOG_ERROR("set_eth_header is not supported for stream_type=socket "
                   "with protocol={}; "
...
Status SocketMgr::set_ipv4_header(BurstParams *burst, int idx, int ip_len,
                                  uint8_t proto, unsigned int src_host,
                                  unsigned int dst_host) {
  if (is_roce_protocol()) {
#if DAQIRI_MGR_RDMA
    return roce_mgr_ != nullptr
               ? roce_mgr_->set_ipv4_header(burst, idx, ip_len, proto, src_host,
                                            dst_host)
  ...
  DAQIRI_LOG_ERROR("set_ipv4_header is not supported for stream_type=socket "
                   "with protocol={}; "
                   "kernel sockets own L2/L3/L4 headers",
                   socket_protocol_to_string(cfg_.common_.protocol));
  return Status::NOT_SUPPORTED;
...
Status SocketMgr::set_udp_header(BurstParams *burst, int idx, int udp_len,
                                 uint16_t src_port, uint16_t dst_port) {
  if (is_roce_protocol()) {
#if DAQIRI_MGR_RDMA
    return roce_mgr_ != nullptr ? roce_mgr_->set_udp_header(burst, idx, udp_len,
                                                            src_port, dst_port)
  ...
  DAQIRI_LOG_ERROR("set_udp_header is not supported for stream_type=socket "
                   "with protocol={}; "
                   "kernel sockets own L2/L3/L4 headers",
                   socket_protocol_to_string(cfg_.common_.protocol));
  return Status::NOT_SUPPORTED;
...
Status SocketMgr::set_udp_payload(BurstParams *burst, int idx, void *data,
                                  int len) {
  if (burst == nullptr || burst->pkts[0] == nullptr || idx < 0 ||
      idx >= static_cast<int>(burst->hdr.hdr.num_pkts) || data == nullptr ||
      len < 0) {
    return Status::INVALID_PARAMETER;
  }
...
#if DAQIRI_MGR_RDMA
    return roce_mgr_ != nullptr
               ? roce_mgr_->set_packet_lengths(burst, idx, lens)
...
Status SocketMgr::set_packet_tx_time(BurstParams *burst, int idx,
                                     uint64_t time) {
  return Status::NOT_SUPPORTED;
}
...
void SocketMgr::clear_rx_queues() {
  for (auto &ep : endpoints_) {
    if (ep == nullptr || ep->rx_queue_state == nullptr) {
      continue;
    }
...
void SocketMgr::shutdown() {
  ...
  for (auto &ep : endpoints_) {
    ...
    if (ep->accept_thread.joinable()) {
      ep->accept_thread.join();
    }
...
void SocketMgr::print_stats() {
  ...
  DAQIRI_LOG_INFO(
      "Socket manager stats: tx_pkts={} tx_bytes={} rx_pkts={} rx_bytes={}",
      tx_pkts_.load(), tx_bytes_.load(), rx_pkts_.load(), rx_bytes_.load());
}
...
BurstParams *SocketMgr::create_tx_burst_params() { return new BurstParams{}; }

Status SocketMgr::pop_rx_burst(const std::shared_ptr<RxQueueState> &qstate,
                               BurstParams **burst) {
  if (burst == nullptr || qstate == nullptr) {
    return Status::INVALID_PARAMETER;
  }
...
void SocketMgr::push_rx_burst(const std::shared_ptr<RxQueueState> &qstate,
                              BurstParams *burst) {
  if (qstate == nullptr || burst == nullptr) {
    return;
  }
  std::lock_guard<std::mutex> lock(qstate->mutex);
...
Status SocketMgr::get_rx_burst(BurstParams **burst, int port, int q) {
  ...
  for (auto &ep : endpoints_) {
    if (ep != nullptr && ep->port == static_cast<uint16_t>(port) &&
        ep->rx_queue == static_cast<uint16_t>(q)) {
...
Status SocketMgr::get_rx_burst(BurstParams **burst, uintptr_t conn_id,
                               bool server) {
  if (is_roce_protocol()) {
#if DAQIRI_MGR_RDMA
    return roce_mgr_ != nullptr
               ? roce_mgr_->get_rx_burst(burst, conn_id, server)
               : roce_not_initialized("get_rx_burst");
#else
    return roce_not_initialized("get_rx_burst");
#endif
...
#if DAQIRI_MGR_RDMA
    return roce_mgr_ != nullptr
               ? roce_mgr_->get_tx_metadata_buffer(burst)
...
const SocketMgr::EndpointState *
SocketMgr::endpoint_for_port(uint16_t port) const {
...
bool SocketMgr::send_tcp_burst(int fd, BurstParams *burst, size_t *sent_pkts,
                               uint64_t *sent_bytes) {
  if (sent_pkts != nullptr) {
    *sent_pkts = 0;
  }
  if (sent_bytes != nullptr) {
    *sent_bytes = 0;
  }
  if (burst == nullptr || burst->pkts[0] == nullptr ||
...
bool SocketMgr::send_udp_burst(EndpointState &ep, BurstParams *burst,
                               size_t *sent_pkts, uint64_t *sent_bytes) {
  if (sent_pkts != nullptr) {
    *sent_pkts = 0;
  }
  if (sent_bytes != nullptr) {
  ...
  while (offset < num_pkts) {
    ...
    for (int i = 0; i < sent; ++i) {
      ...
      if (sent_bytes != nullptr) {
        *sent_bytes += static_cast<uint64_t>(batch[i].msg_len);
      }
    }
...
Status SocketMgr::send_tx_burst(BurstParams *burst) {
  if (is_roce_protocol()) {
#if DAQIRI_MGR_RDMA
    return roce_mgr_ != nullptr ? roce_mgr_->send_tx_burst(burst)
                                : roce_not_initialized("send_tx_burst");
#else
  ...
  if (ep == nullptr) {
    DAQIRI_LOG_ERROR("No socket endpoint configured for port {}",
  ...
  if (cfg_.common_.protocol == SocketProtocol::UDP) {
    ...
    if (sent_bytes > 0) {
      tx_bytes_.fetch_add(sent_bytes);
    }
...
bool SocketMgr::validate_config() const {
  ...
  if (cfg_.common_.protocol != SocketProtocol::TCP &&
    ...
    DAQIRI_LOG_ERROR(
        "Socket manager supports tcp/udp directly, or roce via delegation");
    return false;
  }
...
std::string SocketMgr::endpoint_key(const std::string &ip,
                                    uint16_t port) const {
  return ip + ":" + std::to_string(port);
}

uintptr_t SocketMgr::next_conn_id() { return next_conn_id_.fetch_add(1); }

std::shared_ptr<SocketMgr::RxQueueState>
SocketMgr::get_or_create_rx_queue(uint16_t port, uint16_t queue) {
  for (auto &ep : endpoints_) {
    if (ep != nullptr && ep->port == port && ep->rx_queue == queue &&
        ep->rx_queue_state != nullptr) {
  ...
  auto apply_queue_mr = [&](const auto &queues) {
    for (const auto &queue : queues) {
      for (const auto &mr_name : queue.common_.mrs_) {
        ...
        max_size =
            std::max(max_size, static_cast<int>(mr_it->second.buf_size_));
      }
    }
  };
...
uint16_t
SocketMgr::select_queue_id(const std::vector<RxQueueConfig> &queues) const {
  if (queues.empty()) {
    return 0;
  }
...
uint16_t
SocketMgr::select_queue_id(const std::vector<TxQueueConfig> &queues) const {
  if (queues.empty()) {
    return 0;
  }
  return static_cast<uint16_t>(queues.front().common_.id_);
...
uint32_t
SocketMgr::select_batch_size(const std::vector<TxQueueConfig> &queues) const {
  if (queues.empty()) {
    return 1;
  }
...
std::shared_ptr<SocketMgr::ConnectionState>
SocketMgr::register_connection(int fd, uint16_t port, uint16_t queue,
                               int if_index, bool server_side, bool is_udp,
                               const std::shared_ptr<RxQueueState> &rx_queue,
...
std::shared_ptr<SocketMgr::ConnectionState>
SocketMgr::create_tcp_client_connection(
  ...
  if (!src_addr.empty() || src_port != 0) {
    const std::string bind_ip =
        src_addr.empty() ? std::string("0.0.0.0") : src_addr;
    ...
    if (::bind(fd, reinterpret_cast<sockaddr *>(&bind_addr),
               sizeof(bind_addr)) != 0) {
      DAQIRI_LOG_ERROR("TCP client bind failed for {}:{}: {}", bind_ip,
  ...
  while (running_.load()) {
    if (::connect(fd, reinterpret_cast<sockaddr *>(&dst), sizeof(dst)) == 0) {
      auto conn = register_connection(fd, ep.port, ep.rx_queue, ep.if_index,
                                      false, false, ep.rx_queue_state, true);
      if (set_as_primary) {
        std::lock_guard<std::mutex> lock(state_mutex_);
        default_connection_by_port_[ep.port] = conn->conn_id;
        ep.primary_conn_id = conn->conn_id;
      }
      DAQIRI_LOG_INFO("Connected TCP client {}:{} on port {}", dst_addr,
                      dst_port, ep.port);
      return conn;
    }

    DAQIRI_LOG_WARN("TCP connect to {}:{} failed: {}. Retrying in {}s",
                    dst_addr, dst_port, strerror(errno), retry_s);
...
void SocketMgr::setup_tcp_endpoint(EndpointState &ep) {
  if (ep.socket_cfg.mode_ == SocketMode::CLIENT) {
    ...
    auto conn = create_tcp_client_connection(ep, ep.socket_cfg.remote_ip_,
                                             ep.socket_cfg.remote_port_, src_ip,
                                             src_port, true);
    if (conn == nullptr) {
      throw std::runtime_error(
          "failed to establish configured TCP client connection");
    }
    return;
  }
  ...
  sockaddr_in bind_addr{};
  if (!parse_ipv4_addr(ep.socket_cfg.local_ip_, ep.socket_cfg.local_port_,
                       &bind_addr)) {
    close_fd(fd);
  ...
  ep.accept_thread =
      std::thread(&SocketMgr::tcp_accept_loop, this, ep.if_index);

  DAQIRI_LOG_INFO("TCP server listening on {}:{} (port={})",
                  ep.socket_cfg.local_ip_, ep.socket_cfg.local_port_, ep.port);
}
...
void SocketMgr::setup_udp_endpoint(EndpointState &ep) {
  ...
  if (ep.socket_cfg.mode_ == SocketMode::SERVER) {
    ...
    if (!ep.socket_cfg.local_ip_.empty() || ep.socket_cfg.local_port_ != 0) {
      const std::string bind_ip = ep.socket_cfg.local_ip_.empty()
                                      ? std::string("0.0.0.0")
    ...
    if (!parse_ipv4_addr(ep.socket_cfg.remote_ip_, ep.socket_cfg.remote_port_,
                         &dst_addr)) {
      close_fd(fd);
      throw std::runtime_error("invalid UDP client destination address");
    }

    if (::connect(fd, reinterpret_cast<sockaddr *>(&dst_addr),
                  sizeof(dst_addr)) != 0) {
      const auto err = std::string(strerror(errno));
      close_fd(fd);
  ...
  auto conn = register_connection(fd, ep.port, ep.rx_queue, ep.if_index,
                                  ep.socket_cfg.mode_ == SocketMode::SERVER,
                                  true, ep.rx_queue_state, false);
  ep.primary_conn_id = conn->conn_id;

  {
    ...
    default_connection_by_port_[ep.port] = conn->conn_id;
    if (ep.socket_cfg.mode_ == SocketMode::SERVER) {
      server_connections_[endpoint_key(ep.address, ep.socket_cfg.local_port_)]
          .push_back(conn->conn_id);
      server_connections_[endpoint_key(ep.socket_cfg.local_ip_,
  ...
  DAQIRI_LOG_INFO(
      ...
      ep.socket_cfg.mode_ == SocketMode::SERVER ? ep.socket_cfg.local_ip_
                                                : ep.socket_cfg.remote_ip_,
      ep.socket_cfg.mode_ == SocketMode::SERVER ? ep.socket_cfg.local_port_
                                                : ep.socket_cfg.remote_port_,
      ep.port);
}

void SocketMgr::tcp_accept_loop(int if_index) {
  if (if_index < 0 || if_index >= static_cast<int>(endpoints_.size())) {
    return;
  }
  ...
  while (running_.load() && ep->accept_running.load()) {
    ...
    int fd = ::accept(ep->listen_fd, reinterpret_cast<sockaddr *>(&client_addr),
                      &client_len);
    if (fd < 0) {
      if (!running_.load() || !ep->accept_running.load()) {
        break;
      }
      if (errno == EINTR) {
        continue;
      ...
      DAQIRI_LOG_WARN("TCP accept failed on {}:{}: {}",
                      ep->socket_cfg.local_ip_, ep->socket_cfg.local_port_,
                      strerror(errno));
      continue;
    }
    ...
    auto conn = register_connection(fd, ep->port, ep->rx_queue, ep->if_index,
                                    true, false, ep->rx_queue_state, true);
    ...
    DAQIRI_LOG_INFO("Accepted TCP connection from {}:{} mapped to port={} "
                    "queue={} conn_id={}",
                    sockaddr_to_ip(client_addr), ntohs(client_addr.sin_port),
                    ep->port, ep->rx_queue, conn->conn_id);
  }
}
...
void SocketMgr::tcp_rx_loop(std::shared_ptr<ConnectionState> conn) {
  ...
  while (running_.load() && conn->running.load()) {
    ...
    if (rx < 0) {
      ...
      DAQIRI_LOG_WARN("TCP recv failed on conn_id={}: {}", conn->conn_id,
                      strerror(errno));
      break;
    }
...
void SocketMgr::udp_rx_loop(int if_index) {
  ...
  while (running_.load()) {
    ...
    const int received =
        ::recvmmsg(ep->udp_fd, msgs.data(),
                   static_cast<unsigned int>(kUdpRxBatch), 0, nullptr);
    if (received < 0) {
      if (errno == EINTR) {
        continue;
      }
      ...
      DAQIRI_LOG_WARN("UDP recvmmsg failed on port {}: {}", ep->port,
                      strerror(errno));
      continue;
    }
    if (received == 0) {
...
Status SocketMgr::socket_connect_to_server(const std::string &dst_addr,
                                           uint16_t dst_port,
                                           uintptr_t *conn_id) {
...
Status SocketMgr::socket_connect_to_server(const std::string &dst_addr,
  ...
  for (auto &ep : endpoints_) {
    if (ep == nullptr || ep->socket_cfg.mode_ != SocketMode::CLIENT) {
      continue;
    }

    if (cfg_.common_.protocol == SocketProtocol::TCP) {
      auto conn = create_tcp_client_connection(*ep, dst_addr, dst_port,
                                               src_addr, 0, true);
      if (conn == nullptr) {
    ...
    if (cfg_.common_.protocol == SocketProtocol::UDP) {
      ...
      if (::connect(ep->udp_fd, reinterpret_cast<sockaddr *>(&dst),
                    sizeof(dst)) != 0) {
        DAQIRI_LOG_ERROR("UDP connect failed for {}:{}: {}", dst_addr, dst_port,
                         strerror(errno));
        return Status::CONNECT_FAILURE;
      }
...
Status SocketMgr::socket_get_port_queue(uintptr_t conn_id, uint16_t *port,
                                        uint16_t *queue) {
  if (is_roce_protocol()) {
...
Status SocketMgr::socket_get_server_conn_id(const std::string &server_addr,
  ...
  if (it == server_connections_.end() || it->second.empty()) {
    return Status::NULL_PTR;
  }
...
Status SocketMgr::rdma_connect_to_server(const std::string &dst_addr,
                                         uint16_t dst_port,
                                         uintptr_t *conn_id) {
...
Status SocketMgr::rdma_connect_to_server(const std::string &dst_addr,
  ...
  return roce_mgr_->rdma_connect_to_server(dst_addr, dst_port, src_addr,
                                           conn_id);
}

Status SocketMgr::rdma_get_port_queue(uintptr_t conn_id, uint16_t *port,
                                      uint16_t *queue) {
  if (!is_roce_protocol() || roce_mgr_ == nullptr) {
...
Status SocketMgr::rdma_get_server_conn_id(const std::string &server_addr,
                                          uint16_t server_port,
                                          uintptr_t *conn_id) {
  if (!is_roce_protocol() || roce_mgr_ == nullptr) {
...
Status SocketMgr::rdma_set_header(BurstParams *burst, RDMAOpCode op_code,
                                  uintptr_t conn_id, bool is_server,
                                  int num_pkts, uint64_t wr_id,
                                  const std::string &local_mr_name) {
  if (!is_roce_protocol() || roce_mgr_ == nullptr) {
    return roce_not_initialized("rdma_set_header");
  }
  return roce_mgr_->rdma_set_header(burst, op_code, conn_id, is_server,
