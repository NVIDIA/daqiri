...
void DpdkStats::Init(const NetworkConfig &cfg) {
  ...
  for (const auto &port : cfg_.ifs_) {
    ...
    if (port_stats.xstats == NULL || port_stats.old_xstats == NULL) {
      DAQIRI_LOG_ERROR("Failed to allocate memory for xstats for port {}", port_id);
    ...
    if (xstats_names == NULL) {
      DAQIRI_LOG_ERROR("Failed to allocate memory for xstats_names for port {}", port_id);
    ...
    DAQIRI_LOG_INFO("Initialized DPDK xstats for port {}, found {} stats", port_id, len);
    if (port_stats.rx_missed_idx >= 0) {
      DAQIRI_LOG_INFO("Found rx_missed counter at index {}", port_stats.rx_missed_idx);
    } else {
    ...
    if (port_stats.rx_mbuf_allocation_errors_idx >= 0) {
      ...
      DAQIRI_LOG_WARN("Could not find mbuf allocation counter for port {}", port_id);
    }
  }
...
void DpdkStats::Run() {
  ...
  if (s != 0) {
    DAQIRI_LOG_ERROR("Stats thread pthread_setaffinity_np({}) failed: {}", core_, s);
    return;
  }

  pthread_setname_np(thread, "DPDK_STATS");
  DAQIRI_LOG_INFO("Starting stats thread on core {}", cfg_.common_.master_core_);
