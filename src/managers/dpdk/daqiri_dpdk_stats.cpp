...
void DpdkStats::Init(const NetworkConfig &cfg) {
  ...
  if (cfg_.common_.loopback_ == LoopbackType::LOOPBACK_TYPE_SW) {
    DAQIRI_LOG_INFO(
        "Skipping DPDK stats initialization for software loopback mode");
    return;
  }

  DAQIRI_LOG_INFO("Initializing DPDK stats");
  ...
  for (const auto &intf : cfg_.ifs_) {
    ...
    for (const auto &q : intf.rx_.queues_) {
      ...
      DAQIRI_LOG_INFO("Port {}, Queue {}: Memory regions: {}", port_id,
                      q.common_.id_, mr_names);
    }
  }
  ...
  for (const auto &port : cfg_.ifs_) {
    ...
    if (len < 0) {
      DAQIRI_LOG_ERROR("rte_eth_xstats_get({}) failed: {}", port_id, len);
      continue;
    }
    ...
    port_stats.len = len;
    port_stats.xstats =
        (struct rte_eth_xstat *)calloc(len, sizeof(struct rte_eth_xstat));
    port_stats.old_xstats =
        (struct rte_eth_xstat *)calloc(len, sizeof(struct rte_eth_xstat));

    if (port_stats.xstats == NULL || port_stats.old_xstats == NULL) {
      DAQIRI_LOG_ERROR("Failed to allocate memory for xstats for port {}",
                       port_id);
    ...
    struct rte_eth_xstat_name *xstats_names =
        (struct rte_eth_xstat_name *)calloc(len,
                                            sizeof(struct rte_eth_xstat_name));
    if (xstats_names == NULL) {
      DAQIRI_LOG_ERROR("Failed to allocate memory for xstats_names for port {}",
                       port_id);
      free(port_stats.xstats);
      free(port_stats.old_xstats);
      continue;
    ...
    if (ret < 0 || ret > len) {
      DAQIRI_LOG_ERROR("rte_eth_xstats_get_names({}) failed: {}", port_id, ret);
      free(xstats_names);
      free(port_stats.xstats);
      free(port_stats.old_xstats);
      continue;
    ...
    for (int i = 0; i < len; i++) {
      ...
      } else if (strcmp(xstats_names[i].name, "rx_mbuf_allocation_errors") ==
                 0) {
      ...
      if (strncmp(xstats_names[i].name, rx_q_errors_prefix,
                  strlen(rx_q_errors_prefix)) == 0) {
        const char *queue_num_str =
            xstats_names[i].name + strlen(rx_q_errors_prefix);
        char *endptr;
        int queue_id = strtol(queue_num_str, &endptr, 10);

        // Verify this is a valid rx_q*_errors counter
        if (endptr != queue_num_str &&
            strcmp(endptr, rx_q_errors_suffix) == 0) {
          if (queue_id >= 0 && queue_id < port_stats.MAX_QUEUE_COUNT) {
            port_stats.rx_queue_errors_idx[queue_id] = i;
            DAQIRI_LOG_INFO("Found rx_q{}_errors counter at index {}", queue_id,
                            i);
          }
        }
      }
    ...
    if (ret < 0 || ret > len) {
      DAQIRI_LOG_ERROR("rte_eth_xstats_get({}) failed: {}", port_id, ret);
      free(xstats_names);
      free(port_stats.xstats);
      free(port_stats.old_xstats);
      continue;
    ...
    memcpy(port_stats.old_xstats, port_stats.xstats,
           len * sizeof(struct rte_eth_xstat));

    // Free xstats_names as we don't need them anymore
    free(xstats_names);

    DAQIRI_LOG_INFO("Initialized DPDK xstats for port {}, found {} stats",
                    port_id, len);
    if (port_stats.rx_missed_idx >= 0) {
      DAQIRI_LOG_INFO("Found rx_missed counter at index {}",
                      port_stats.rx_missed_idx);
    } else {
      DAQIRI_LOG_WARN("Could not find rx_missed counter for port {}", port_id);
    }

    if (port_stats.rx_mbuf_allocation_errors_idx >= 0) {
      DAQIRI_LOG_INFO("Found mbuf allocation counter at index {}",
                      port_stats.rx_mbuf_allocation_errors_idx);
    } else {
      DAQIRI_LOG_WARN("Could not find mbuf allocation counter for port {}",
                      port_id);
    }
  }
  ...
  DAQIRI_LOG_INFO("Initialized DPDK stats");
}
...
void DpdkStats::Run() {
  ...
  if (cfg_.common_.loopback_ == LoopbackType::LOOPBACK_TYPE_SW) {
    DAQIRI_LOG_INFO("Skipping DPDK stats thread for software loopback mode");
    return;
  }
  ...
  if (s != 0) {
    DAQIRI_LOG_ERROR("Stats thread pthread_setaffinity_np({}) failed: {}",
                     core_, s);
    return;
  }

  pthread_setname_np(thread, "DPDK_STATS");
  DAQIRI_LOG_INFO("Starting stats thread on core {}",
                  cfg_.common_.master_core_);
    ...
    for (const auto &port : cfg_.ifs_) {
      ...
      if (port_stats.rx_mbuf_allocation_errors_idx >= 0) {
        ...
        uint64_t old_rx_mbuf_alloc_err =
            port_stats.old_xstats[port_stats.rx_mbuf_allocation_errors_idx]
                .value;

        if (rx_mbuf_alloc_err > old_rx_mbuf_alloc_err) {
          const auto alloc_err_since_last_check =
              rx_mbuf_alloc_err - old_rx_mbuf_alloc_err;
          DAQIRI_LOG_DEBUG("Port {}: Buffer allocation errors since last poll "
                           "{}ms ago: {}, total {}. Software is not keeping up "
                           "with the NIC RX rate.",
                           interface_name, POLLING_INTERVAL_MS,
                           alloc_err_since_last_check, rx_mbuf_alloc_err);
        }
      }
      ...
      for (const auto &[queue_id, error_idx] : port_stats.rx_queue_errors_idx) {
        ...
        if (rx_queue_errors > old_rx_queue_errors) {
          ...
          DAQIRI_LOG_WARN("'{}' interface ({}), Rx '{}' queue ({}): "
                          "Packets to '{}' might get dropped. Either CPU #{} "
                          "couldn't read from the NIC fast "
                          "enough, downstream software was not processing and "
                          "freeing packets fast enough, "
                          "or not enough buffers were allocated in this memory "
                          "region to begin with.",
                          interface_name, port_id, q_cfg.name_, queue_id,
                          mr_names, q_cfg.cpu_core_);
        }
      }
      ...
      if (port_stats.rx_missed_idx >= 0) {
        ...
        uint64_t old_rx_missed =
            port_stats.old_xstats[port_stats.rx_missed_idx].value;

        if (rx_missed > old_rx_missed) {
          const auto missed_since_last_check = rx_missed - old_rx_missed;
          DAQIRI_LOG_ERROR("'{}' interface ({}), Rx: Dropped {} packets since "
                           "last poll {}ms ago (total: {})",
                           interface_name, port_id, missed_since_last_check,
                           POLLING_INTERVAL_MS, rx_missed);
        }
      }
