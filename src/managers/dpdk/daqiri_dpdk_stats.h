...
public:
  ...
  void Stop() { force_quit_.store(true); }
...
private:
  ...
  struct PortXStats {
    ...
    std::unordered_map<std::string, int>
        name_to_idx;                   // Map from stat name to index
    ...
    static constexpr int MAX_QUEUE_COUNT =
        128; // Maximum number of queues supported
    std::unordered_map<int, int>
        rx_queue_errors_idx; // Indices for rx_q*_errors counters

    PortXStats()
        : len(0), xstats(nullptr), old_xstats(nullptr), rx_missed_idx(-1),
          rx_mbuf_allocation_errors_idx(-1) {
