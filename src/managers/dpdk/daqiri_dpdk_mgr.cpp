...
static inline uint32_t generate_queue_key(int port_id, int queue_id) {
  return (static_cast<uint32_t>(port_id) << 16) |
...
static inline bool is_cpu_accessible_memory(MemoryKind kind) {
  return kind == MemoryKind::HOST || kind == MemoryKind::HOST_PINNED ||
         kind == MemoryKind::HUGE;
}

static inline uint32_t
extract_bits_be_host(const void *data, uint16_t bit_offset, uint8_t bit_width) {
  const auto *bytes = static_cast<const uint8_t *>(data);
  uint32_t value = 0;
  for (uint8_t i = 0; i < bit_width; ++i) {
    const uint32_t bit_idx =
        static_cast<uint32_t>(bit_offset) + static_cast<uint32_t>(i);
...
derive_reorder_batch_id_host(const ReorderConfig &cfg, const void *pkt_ptr,
  ...
  const uint32_t seq = extract_bits_be_host(
      pkt_ptr, get_reorder_seq_bit_offset(cfg), get_reorder_seq_bit_width(cfg));
  return packets_per_batch == 0
             ? 0U
             : static_cast<uint64_t>(seq / packets_per_batch);
}

static inline bool reorder_uses_data_type_conversion(const ReorderConfig &cfg) {
  if (!cfg.data_types_.enabled_) {
    return false;
  }
  if (cfg.data_types_.input_type_ != cfg.data_types_.output_type_) {
    return true;
  }
...
static inline bool
compute_reorder_output_payload_len(const ReorderConfig &cfg,
                                   uint32_t input_payload_len,
                                   uint32_t *output_payload_len) {
  if (output_payload_len == nullptr) {
    return false;
  }
  ...
  const uint32_t input_bits =
      reorder_data_type_bit_width(cfg.data_types_.input_type_);
  const uint32_t output_bits =
      reorder_data_type_bit_width(cfg.data_types_.output_type_);
  if (input_bits == 0 || output_bits == 0) {
    return false;
  }

  const uint64_t total_input_bits =
      static_cast<uint64_t>(input_payload_len) * 8ULL;
  if ((total_input_bits % static_cast<uint64_t>(input_bits)) != 0ULL) {
    return false;
  }

  const uint64_t element_count =
      total_input_bits / static_cast<uint64_t>(input_bits);
  const uint64_t total_output_bits =
      element_count * static_cast<uint64_t>(output_bits);
  if ((total_output_bits % 8ULL) != 0ULL) {
    return false;
  }
...
static inline bool
compute_reorder_max_output_payload_len(const ReorderConfig &cfg,
  ...
  const uint64_t total_input_bits =
      static_cast<uint64_t>(max_input_payload_len) * 8ULL;
  const uint64_t element_count =
      total_input_bits / static_cast<uint64_t>(input_bits);
  const uint64_t total_output_bits =
      element_count * static_cast<uint64_t>(output_bits);
...
struct TxWorkerParams {
  ...
  struct rte_ring
      *lb_ring; // Ring used between TX and RX when in SW loopback mode
...
struct RxWorkerMultiQParams {
  ...
  struct rte_mempool *flowid_pool;
  struct rte_mempool
      *rx_burst_pool; // Pool used to pull out bursts from RX pool
  struct rte_mempool *
      tx_burst_pool; // Pool used for loopback mode to return transmitted bursts
  struct rte_mempool *rx_meta_pool; // Pool used for RX metadata structures
  struct rte_mempool
      *tx_meta_pool; // Pool used in loopback for returning transmitted metadata
  int rx_meta_pool_size;
};
...
bool DpdkMgr::init_reorder_queue_state(const InterfaceConfig &intf,
  ...
  for (const auto &flow : intf.rx_.flows_) {
    if (flow_id_to_queue.find(flow.id_) != flow_id_to_queue.end()) {
      DAQIRI_LOG_ERROR("Duplicate flow ID {} in interface '{}'", flow.id_,
                       intf.name_);
      return false;
    }
  ...
  for (const auto &reorder_cfg : intf.rx_.reorder_configs_) {
    ...
    for (const auto flow_id : reorder_cfg.flow_ids_) {
      ...
      if (flow_it == flow_id_to_queue.end()) {
        DAQIRI_LOG_ERROR("Reorder config '{}' references unknown flow ID {} on "
                         "interface '{}'",
                         reorder_cfg.name_, flow_id, intf.name_);
        return false;
      }
      ...
      if (flow_queue_id < 0) {
        ...
        DAQIRI_LOG_ERROR("Reorder config '{}' has flow IDs mapped to multiple "
                         "queues on interface '{}'. "
                         "Each reorder config must map to a single RX queue",
                         reorder_cfg.name_, intf.name_);
        return false;
      }
    ...
    if (queue_flow_ids.empty()) {
      continue;
    }

    if (qcfg.common_.mrs_.size() != 1) {
      DAQIRI_LOG_ERROR("Queue '{}'/{} using reorder config '{}' must define "
                       "exactly one source memory "
                       "region. Header-data split reordering is not supported.",
                       qcfg.common_.name_, qcfg.common_.id_, reorder_cfg.name_);
      return false;
    }
    ...
    if (source_mr_it == cfg_.mrs_.end()) {
      DAQIRI_LOG_ERROR("Queue '{}' references unknown memory region '{}'",
                       qcfg.common_.name_, source_mr_name);
      return false;
    }
    ...
    if (use_gpu_backend && !is_cuda_accessible_packet_memory(source_mr.kind_)) {
      DAQIRI_LOG_ERROR("Queue '{}' with reorder config '{}' must use DEVICE or "
                       "HOST_PINNED packet memory "
                       "for GPU reorder",
                       qcfg.common_.name_, reorder_cfg.name_);
      return false;
    }
    if (!use_gpu_backend && !is_cpu_accessible_memory(source_mr.kind_)) {
      DAQIRI_LOG_ERROR("Queue '{}' with reorder config '{}' must use HOST, "
                       "HOST_PINNED, or HUGE packet "
    ...
    if (out_mr_it == cfg_.mrs_.end()) {
      DAQIRI_LOG_ERROR(
          "Reorder config '{}' references unknown output memory region '{}'",
          reorder_cfg.name_, reorder_cfg.memory_region_);
      return false;
    }
    ...
    if (use_gpu_backend && out_mr.kind_ != MemoryKind::DEVICE &&
      ...
      DAQIRI_LOG_ERROR("Reorder output memory region '{}' in config '{}' must "
                       "be DEVICE or HOST_PINNED memory",
                       reorder_cfg.memory_region_, reorder_cfg.name_);
      return false;
    }
    if (!use_gpu_backend && !is_cpu_accessible_memory(out_mr.kind_)) {
      DAQIRI_LOG_ERROR("Reorder output memory region '{}' in config '{}' must "
                       "be HOST, HOST_PINNED, or HUGE "
                       "memory for CPU reorder",
                       reorder_cfg.memory_region_, reorder_cfg.name_);
      return false;
    }

    if (use_gpu_backend && copy_src_mr->kind_ == MemoryKind::DEVICE &&
        out_mr.kind_ == MemoryKind::DEVICE &&
        out_mr.affinity_ != copy_src_mr->affinity_) {
      DAQIRI_LOG_ERROR("Reorder config '{}' requires input/output memory on "
                       "the same GPU (src affinity {} "
                       "!= reorder affinity {})",
                       reorder_cfg.name_, copy_src_mr->affinity_,
                       out_mr.affinity_);
      return false;
    }

    if (reorder_cfg.payload_byte_offset_ >= logical_source_size) {
      DAQIRI_LOG_ERROR("Reorder config '{}' payload_byte_offset {} is out of "
                       "range for source buffer size {}",
                       reorder_cfg.name_, reorder_cfg.payload_byte_offset_,
                       logical_source_size);
      return false;
    }
    ...
    if (reorder_cfg.method_ == ReorderMethod::SEQ_BATCH_NUMBER) {
      ...
      DAQIRI_LOG_ERROR("Reorder config '{}' has invalid method",
                       reorder_cfg.name_);
      return false;
    }
    ...
    if (packets_per_batch == 0 ||
      ...
      DAQIRI_LOG_ERROR("Reorder config '{}' packets_per_batch {} must be in "
                       "range [1, queue batch_size {}]",
                       reorder_cfg.name_, packets_per_batch,
                       qcfg.common_.batch_size_);
      return false;
    }

    if (copy_source_offset >= copy_src_mr->buf_size_) {
      DAQIRI_LOG_ERROR("Reorder config '{}' copy source offset {} is out of "
                       "range for source MR '{}' size {}",
                       reorder_cfg.name_, copy_source_offset,
                       copy_src_mr->name_, copy_src_mr->buf_size_);
      return false;
    }
    ...
    if (!compute_reorder_max_output_payload_len(reorder_cfg, slot_stride,
                                                &output_slot_stride)) {
      DAQIRI_LOG_ERROR(
          "Reorder config '{}' cannot convert source slot size {} from {} to "
          "{}",
          reorder_cfg.name_, slot_stride,
    ...
    const bool use_data_type_conversion =
        reorder_uses_data_type_conversion(reorder_cfg);
    if (use_data_type_conversion && !use_gpu_backend) {
      DAQIRI_LOG_ERROR("Reorder config '{}' data type conversion is supported "
                       "only for gpu reorder_type",
                       reorder_cfg.name_);
      return false;
    }

    const uint64_t min_required_out =
        static_cast<uint64_t>(output_slot_stride) *
        static_cast<uint64_t>(packets_per_batch);
    if (min_required_out > out_mr.buf_size_) {
      DAQIRI_LOG_ERROR(
          ...
          "{})",
          out_mr.name_, out_mr.buf_size_, reorder_cfg.name_, min_required_out,
          packets_per_batch, output_slot_stride);
      return false;
    }
    ...
    const auto enable_cuda_events =
        [&destroy_cuda_events](ReorderOutputPool &pool, int device_id) -> bool {
      if (pool.cuda_events_enabled) {
        return true;
      }
      cudaSetDevice(device_id);
      for (size_t i = 0; i < pool.buffers.size(); ++i) {
        auto &buffer = pool.buffers[i];
        if (cudaEventCreateWithFlags(&buffer.event, cudaEventDisableTiming) !=
            cudaSuccess) {
          for (auto &created : pool.buffers) {
            destroy_cuda_events(created);
          }
          return false;
        }
        if (cudaHostAlloc(reinterpret_cast<void **>(&buffer.h_batch_id),
            ...
            cudaMalloc(reinterpret_cast<void **>(&buffer.d_batch_id),
    ...
    if (pool_it == reorder_output_pools_.end()) {
      ...
      if (ar_it == ar_.end()) {
        DAQIRI_LOG_ERROR("No allocated memory found for reorder MR '{}'",
                         out_mr.name_);
        return false;
      }
      ...
      if (use_gpu_backend &&
          !enable_cuda_events(*output_pool, cuda_device_id)) {
        DAQIRI_LOG_ERROR(
            "Failed to create CUDA events for reorder output MR '{}'",
            out_mr.name_);
        return false;
      }
      ...
      if (use_gpu_backend && output_pool->cuda_events_enabled &&
          output_pool->cuda_device_id != cuda_device_id) {
        DAQIRI_LOG_ERROR("Reorder output MR '{}' is shared by configs using "
                         "different CUDA devices ({} != {})",
                         out_mr.name_, output_pool->cuda_device_id,
                         cuda_device_id);
        return false;
      }
      if (use_gpu_backend &&
    ...
    if (use_gpu_backend) {
      ...
      if (cudaMalloc(reinterpret_cast<void **>(&plan.d_input_ptrs),
                     sizeof(void *) * plan.cuda_staging_capacity) !=
          cudaSuccess) {
        DAQIRI_LOG_ERROR(
            "Failed to allocate CUDA staging buffers for reorder config '{}'",
            reorder_cfg.name_);
    ...
    for (const auto flow_id : queue_flow_ids) {
      if (qstate.flow_id_to_plan.find(flow_id) !=
          qstate.flow_id_to_plan.end()) {
        DAQIRI_LOG_ERROR("Flow ID {} is mapped to multiple reorder configs on "
                         "interface '{}' queue {}",
                         flow_id, intf.name_, qcfg.common_.id_);
        return false;
      }
  ...
  if (!qstate.plans.empty()) {
    ...
    for (auto &idxs : qstate.plan_pkt_indices) {
      idxs.resize(queue_batch_size);
    }
...
void DpdkMgr::cleanup_reorder_state() {
  for (auto &[qkey, qstate] : reorder_queue_states_) {
    ...
    for (auto &plan : qstate.plans) {
      ...
      if (plan.gpu_profile.gpu_kernel_samples != 0) {
        ...
        DAQIRI_LOG_INFO("Reorder GPU profile '{}' kernel_samples={} "
                        "avg_kernel_us={:.2f} max_kernel_us={:.2f}",
                        plan.config != nullptr ? plan.config->name_
                                               : "<unknown>",
                        plan.gpu_profile.gpu_kernel_samples, avg_kernel_us,
                        plan.gpu_profile.gpu_kernel_max_us);
      }
#endif
      if (plan.direct_arrival_batch.packet_count != 0 &&
          !plan.h_source_mbufs.empty()) {
      ...
      for (auto &pending : plan.pending_copies) {
        if (pending.output_pool != nullptr &&
            pending.buffer_idx < pending.output_pool->buffers.size()) {
  ...
  for (auto &[mr_name, output_pool] : reorder_output_pools_) {
    ...
    if (output_pool->cuda_events_enabled) {
...
Status DpdkMgr::acquire_reorder_output_buffer(ReorderPlanRuntime &plan,
                                              size_t *buffer_idx,
                                              void **output_buffer) {
  if (buffer_idx == nullptr || output_buffer == nullptr) {
    return Status::NULL_PTR;
  }
  *output_buffer = nullptr;
  if (plan.output_pool == nullptr || plan.output_pool->buffers.empty()) {
    DAQIRI_LOG_ERROR("Reorder config '{}' has no output buffers",
                     plan.config->name_);
  ...
  DAQIRI_LOG_ERROR("No free reorder output buffers for MR '{}'",
                   plan.memory_region_name);
  return Status::NO_FREE_PACKET_BUFFERS;
}

void DpdkMgr::release_reorder_output_buffer(
    std::shared_ptr<ReorderOutputPool> output_pool, size_t buffer_idx) {
  if (output_pool == nullptr || buffer_idx >= output_pool->buffers.size()) {
    return;
  }
...
Status DpdkMgr::poll_reorder_events(ReorderPlanRuntime &plan) {
  ...
  for (auto it = plan.pending_copies.begin();
       it != plan.pending_copies.end();) {
    auto &pending = *it;
    if (pending.output_pool == nullptr ||
        pending.buffer_idx >= pending.output_pool->buffers.size()) {
    ...
    if (event_status != cudaSuccess) {
      DAQIRI_LOG_ERROR(
          "Failed while polling reorder CUDA event for config '{}'",
          plan.config->name_);
    ...
    if (buffer.kernel_start_event != nullptr &&
      ...
      const cudaError_t elapsed_status = cudaEventElapsedTime(
          &kernel_ms, buffer.kernel_start_event, buffer.kernel_stop_event);
      if (elapsed_status == cudaSuccess) {
        ...
        DAQIRI_LOG_ERROR(
            "Failed to compute reorder kernel elapsed time for config '{}'",
            plan.config->name_);
        (void)cudaGetLastError();
        final_status = Status::INTERNAL_ERROR;
      }
...
Status DpdkMgr::append_reorder_packet(ReorderPlanRuntime &plan,
                                      struct rte_mbuf *mbuf, void *pkt_ptr,
                                      uint64_t now_cycles, size_t *batch_size) {
  ...
  *batch_size = 0;
  ...
  if (packet_idx >= plan.h_input_ptrs.size() ||
      packet_idx >= plan.h_source_mbufs.size()) {
  ...
  if (packet_idx == 0) {
    ...
    uint32_t copy_len = 0;
    if (pkt_len > plan.payload_byte_offset) {
      copy_len = pkt_len - plan.payload_byte_offset;
    }
    ...
    if (!compute_reorder_output_payload_len(
      ...
      DAQIRI_LOG_ERROR("Reorder config '{}' cannot convert packet payload size "
                       "{} from {} to {}",
                       plan.config->name_, batch.input_payload_len,
...
Status DpdkMgr::create_reorder_output_burst(
  ...
  *out_burst = nullptr;
  ...
  burst->hdr.hdr.burst_flags =
      kBurstFlagDpdkReordered |
      (timeout_flush ? kBurstFlagDpdkReorderTimeout : 0U);
  ...
  static_assert(sizeof(ReorderBurstInfo) <= ADV_NETWORK_HEADER_SIZE_BYTES -
                                                sizeof(void *) -
...
Status DpdkMgr::flush_reorder_batch(ReorderPlanRuntime &plan, uint32_t batch_id,
                                    bool timeout_flush,
  ...
  *out_burst = nullptr;
  ...
  const uint64_t aggregate_len64 = static_cast<uint64_t>(output_slots) *
                                   static_cast<uint64_t>(output_payload_len);
  if (aggregate_len64 >
      static_cast<uint64_t>(std::numeric_limits<uint32_t>::max())) {
    DAQIRI_LOG_ERROR("Reorder output length {} is too large for config '{}'",
                     aggregate_len64, plan.config->name_);
    return Status::INVALID_PARAMETER;
  }
  ...
  Status status =
      acquire_reorder_output_buffer(plan, &output_buffer_idx, &output_buffer);
  ...
  if (plan.use_gpu_backend) {
    ...
    if (output_state.source_mbufs.size() < num_pkts) {
      DAQIRI_LOG_ERROR("Reorder output buffer has insufficient source packet "
                       "tracking storage");
      release_reorder_output_buffer(plan.output_pool, output_buffer_idx);
      return Status::INTERNAL_ERROR;
    }
    std::memcpy(output_state.source_mbufs.data(), plan.h_source_mbufs.data(),
                sizeof(struct rte_mbuf *) * num_pkts);
    output_state.source_packet_count = num_pkts;
    if (output_state.h_batch_id == nullptr ||
        output_state.d_batch_id == nullptr) {
      DAQIRI_LOG_ERROR(
          "Reorder output buffer has no GPU batch metadata storage");
      release_reorder_output_buffer(plan.output_pool, output_buffer_idx);
      return Status::INTERNAL_ERROR;
    }
    output_batch_id_host = output_state.h_batch_id;

    if (cudaMemcpyAsync(plan.d_input_ptrs, input_ptrs.data(),
                        sizeof(void *) * num_pkts, cudaMemcpyHostToDevice,
                        plan.stream) != cudaSuccess) {
      DAQIRI_LOG_ERROR(
          "Failed to copy ordered reorder packet pointers to device");
      release_reorder_output_buffer(plan.output_pool, output_buffer_idx);
      return Status::INTERNAL_ERROR;
    }

#if DAQIRI_REORDER_GPU_PROFILE
    if (output_state.kernel_start_event == nullptr ||
        output_state.kernel_stop_event == nullptr) {
      DAQIRI_LOG_ERROR(
          "Reorder output buffer has no CUDA backend timing events");
      release_reorder_output_buffer(plan.output_pool, output_buffer_idx);
      return Status::INTERNAL_ERROR;
    }
    if (cudaEventRecord(output_state.kernel_start_event, plan.stream) !=
        cudaSuccess) {
    ...
    packet_reorder_copy_payload_by_sequence(
        output_buffer, reinterpret_cast<const void *const *>(plan.d_input_ptrs),
        input_payload_len, output_payload_len, plan.copy_source_offset,
        num_pkts, get_reorder_seq_bit_offset(cfg),
        ...
        plan.data_type_conversion_enabled
            ? static_cast<uint8_t>(plan.input_data_type)
            : 0U,
        plan.data_type_conversion_enabled
            ? static_cast<uint8_t>(plan.output_data_type)
            : 0U,
        plan.data_type_conversion_enabled
            ? static_cast<uint8_t>(plan.input_endianness)
            : 0U,
        output_state.d_batch_id, plan.stream);
...
#if DAQIRI_REORDER_GPU_PROFILE
    if (cudaEventRecord(output_state.kernel_stop_event, plan.stream) !=
        cudaSuccess) {
    ...
    if (cudaMemcpyAsync(output_state.h_batch_id, output_state.d_batch_id,
    ...
    for (uint32_t i = 0; i < num_pkts; ++i) {
      ...
      if (!output_batch_id_ready) {
        output_batch_id =
            derive_reorder_batch_id_host(cfg, src_pkt, plan.packets_per_batch);
        output_batch_id_ready = true;
      }
      const uint32_t seq =
          extract_bits_be_host(src_pkt, get_reorder_seq_bit_offset(cfg),
                               get_reorder_seq_bit_width(cfg));
      const uint32_t slot_idx = seq % plan.packets_per_batch;
      if (slot_idx >= output_slots) {
        continue;
      }

      std::memcpy(out_bytes +
                      (static_cast<size_t>(slot_idx) * output_payload_len),
                  src_pkt + plan.copy_source_offset, output_payload_len);
    }
  }

  status = create_reorder_output_burst(
      plan, plan.output_pool, output_buffer_idx, output_buffer, aggregate_len,
      num_pkts, output_payload_len, output_batch_id, output_batch_id_ready,
      output_batch_id_host, copy_done, timeout_flush, out_burst);
  if (status != Status::SUCCESS) {
    release_reorder_output_buffer(plan.output_pool, output_buffer_idx);
    return status;
  }
  ...
  if (plan.use_gpu_backend) {
    ...
    rte_pktmbuf_free_bulk(plan.h_source_mbufs.data(),
                          static_cast<unsigned int>(num_pkts));
  }
...
Status DpdkMgr::flush_reorder_timeouts(ReorderQueueState &qstate,
                                       uint64_t now_cycles) {
  Status final_status = Status::SUCCESS;
...
Status DpdkMgr::process_burst_for_reorder(uint32_t key,
                                          ReorderQueueState &qstate,
                                          BurstParams *burst) {
  (void)key;
  if (burst == nullptr) {
  ...
  if (qstate.single_plan_fast_path && qstate.plans.size() == 1) {
    ...
    if (plan.use_gpu_backend && plan.stream == nullptr) {
      DAQIRI_LOG_ERROR("Reorder stream is not set for interface port {} queue "
                       "{} config '{}'",
                       plan.port_id, plan.queue_id, plan.config->name_);
  ...
  for (int i = 0; i < num_pkts; ++i) {
    ...
    if (plan_idx >= qstate.plans.size()) {
      DAQIRI_LOG_ERROR("Invalid reorder plan index {} for flow {}", plan_idx,
                       flow_id);
      final_status = Status::INTERNAL_ERROR;
      continue;
    }
    if (qstate.plans[plan_idx].use_gpu_backend &&
        qstate.plans[plan_idx].stream == nullptr) {
      DAQIRI_LOG_ERROR("Reorder stream is not set for interface port {} queue "
                       "{} config '{}'",
    ...
    if (plan_count >= plan_indices.size()) {
      DAQIRI_LOG_ERROR(
          "Reorder plan packet scratch storage is full for plan {}", plan_idx);
      final_status = Status::INTERNAL_ERROR;
      continue;
    }
  ...
  for (size_t plan_idx = 0; plan_idx < qstate.plan_pkt_indices.size();
       ++plan_idx) {
  ...
  if (unmatched_count > 0) {
    auto *flow_info =
        reinterpret_cast<ExtraRxPacketInfo *>(burst->pkt_extra_info);
    ...
    for (int out_idx = 0; out_idx < unmatched_count; ++out_idx) {
      ...
      if (flow_info != nullptr) {
        flow_info[out_idx] = flow_info[in_idx];
      }
    }
...
Status DpdkMgr::get_next_output_or_ready(uint32_t key,
                                         ReorderQueueState &qstate,
                                         BurstParams **burst) {
  if (burst == nullptr) {
    return Status::NULL_PTR;
  ...
  *burst = nullptr;
  ...
  Status timeout_status =
      flush_reorder_timeouts(qstate, rte_get_timer_cycles());
  if (timeout_status != Status::SUCCESS) {
  ...
  while (true) {
    ...
    if (rte_ring_dequeue(ring_it->second, reinterpret_cast<void **>(&raw)) <
        0) {
      break;
    }
...
void DpdkMgr::release_reorder_output_context(BurstParams *burst) {
  ...
  auto ctx =
      std::static_pointer_cast<ReorderBurstContext>(burst->custom_pkt_data);
  if (!ctx || ctx->released) {
    return;
  }
...
Status DpdkMgr::get_reorder_burst_info(BurstParams *burst,
...
Status DpdkMgr::set_reorder_cuda_stream(const std::string &interface_name,
  ...
  if (port < 0) {
    DAQIRI_LOG_ERROR("Invalid interface '{}' in set_reorder_cuda_stream",
                     interface_name);
    return Status::INVALID_PARAMETER;
  }
  ...
  bool found = false;
  for (auto &[qkey, qstate] : reorder_queue_states_) {
    if (get_port_from_key(qkey) != port) {
      continue;
    }
    for (auto &plan : qstate.plans) {
...
Status DpdkMgr::get_mac_addr(int port, char *mac) {
  ...
  memcpy(mac, reinterpret_cast<char *>(&mac_addrs[port]),
         sizeof(mac_addrs[port]));
  return Status::SUCCESS;
}
...
void DpdkMgr::adjust_memory_regions() {
  for (auto &mr : cfg_.mrs_) {
    ...
    DAQIRI_LOG_INFO("Adjusting buffer size to {} for headroom",
                    mr.second.adj_size_);
  }
}
...
void DpdkMgr::setup_accurate_send_scheduling_mask() {
  ...
  if (timestamp_offset_ < 0) {
    DAQIRI_LOG_CRITICAL("{} registration error: {}",
                        RTE_MBUF_DYNFIELD_TIMESTAMP_NAME,
...
void DpdkMgr::create_dummy_rx_q() {
  for (auto &intf : cfg_.ifs_) {
    ...
    if (rx.queues_.size() == 0) {
      DAQIRI_LOG_INFO("Port {} has no RX queues. Creating dummy queue.",
                      intf.port_id_);
...
void DpdkMgr::create_dummy_tx_q() {
  for (auto &intf : cfg_.ifs_) {
    ...
    if (tx.queues_.size() == 0) {
      DAQIRI_LOG_INFO("Port {} has no TX queues. Creating dummy queue.",
                      intf.port_id_);
      const std::string mr_name =
          "MR_Unused_TX_P" + std::to_string(intf.port_id_);
      TxQueueConfig tmp_q;
      tmp_q.common_.name_ =
          "UNUSED_TX_P" + std::to_string(intf.port_id_) + "_Q0";
      tmp_q.common_.id_ = 0;
      tmp_q.common_.batch_size_ = 1;
      tmp_q.common_.split_boundary_ = 0;
      tmp_q.common_.cpu_core_ = "0";
...
void DpdkMgr::initialize() {
  ...
  if (loopback_ == LoopbackType::LOOPBACK_TYPE_SW) {
    ...
    if (cfg_.ifs_[0].rx_.queues_.size() > 1 ||
        cfg_.ifs_[0].tx_.queues_.size() > 1) {
    ...
    if (loopback_ != LoopbackType::LOOPBACK_TYPE_SW) {
      if (looks_like_mac_address(intf.address_)) {
        DAQIRI_LOG_CRITICAL(
            ...
            "DPDK expects a PCI BDF for interfaces[].address (e.g. "
            "0000:17:00.0).",
            intf.name_, intf.address_);
        return;
      }
      if (!looks_like_pci_bdf(intf.address_)) {
        DAQIRI_LOG_CRITICAL("Interface '{}' has invalid address '{}'. DPDK "
                            "expects a PCI BDF in the format "
                            "dddd:bb:dd.f (example: 0000:17:00.0). Remove "
                            "placeholders like '<...>'.",
                            intf.name_, intf.address_);
        return;
      }
    } else if (intf.address_ != "loopback" &&
               !looks_like_pci_bdf(intf.address_)) {
      DAQIRI_LOG_CRITICAL("Interface '{}' has invalid loopback address '{}'. "
                          "In software loopback mode use "
  ...
  num_ports = ifs.size();
  DAQIRI_LOG_INFO("Attempting to use {} ports for high-speed network",
                  num_ports);
  ...
  DAQIRI_LOG_INFO("Setting DPDK log level to: {}",
                  DpdkLogLevel::to_description_string(
                      DpdkLogLevel::from_ano_log_level(cfg_.log_level_)));
  ...
  if (ret < 0) {
    DAQIRI_LOG_CRITICAL("Invalid EAL arguments: errno={} ({})", rte_errno,
                        rte_strerror(rte_errno));
    if (rte_errno == EACCES || rte_errno == EPERM) {
      DAQIRI_LOG_CRITICAL("DPDK permission error. Verify hugepages are "
                          "configured and mounted, container has "
                          "required privileges (vfio/uio), and NIC PCI devices "
                          "are bound to a DPDK-compatible driver.");
    }
    return;
  }
  ...
  for (auto &intf : cfg_.ifs_) {
    if (loopback_ == LoopbackType::LOOPBACK_TYPE_SW) {
      intf.port_id_ = 0;
    } else if (rte_eth_dev_get_port_by_name(intf.address_.c_str(),
                                            &intf.port_id_) < 0) {
      DAQIRI_LOG_CRITICAL("Failed to get port number for {} ({})", intf.name_,
                          intf.address_);
      return;
    }
    DAQIRI_LOG_INFO("{} ({}): identified as port {}", intf.name_, intf.address_,
                    intf.port_id_);
  }
  ...
  for (auto &intf : cfg_.ifs_) {
    ...
    port_q_num[intf.port_id_] = {intf.rx_.queues_.size(),
                                 intf.tx_.queues_.size()};
    port_id_to_name[intf.port_id_] = intf.address_;

    DAQIRI_LOG_INFO("DPDK init ({}) -- RX: {} TX: {}", intf.address_,
    ...
    for (auto &q : rx.queues_) {
      ...
      for (int mr_num = 0; mr_num < q.common_.mrs_.size(); mr_num++) {
        std::string append = "_P" + std::to_string(intf.port_id_) + "_Q" +
                             std::to_string(q.common_.id_) + "_MR" +
                             std::to_string(mr_num);
        std::string pool_name = std::string("RXP") + append;
        const auto &mr = cfg_.mrs_[q.common_.mrs_[mr_num]];

        if (loopback_ !=
            LoopbackType::LOOPBACK_TYPE_SW) { // Loopback needs no RX pools
          ...
          if (pool == nullptr) {
            DAQIRI_LOG_CRITICAL("Could not create external memory mempool {}: "
                                "mbufs={} elsize={} ptr={}",
                                pool_name, mr.num_bufs_, mr.adj_size_,
                                (void *)pool);
            return;
          }
          ...
          DAQIRI_LOG_INFO("Created mempool {} : mbufs={} elsize={} ptr={}",
                          pool_name, mr.num_bufs_, mr.adj_size_, (void *)pool);
      ...
      if (q.common_.mrs_.size() == 1 && q_packet_size > 0) {
        ...
        min_single_seg_rx_buf_size =
            std::min(min_single_seg_rx_buf_size, q_packet_size);
      }
      ...
      if (q.common_.mrs_.size() > 1) {
        ...
        for (int seg = 0; seg < q.common_.mrs_.size(); seg++) {
          ...
          rx_seg->length = (seg == (q.common_.mrs_.size() - 1))
                               ? 0
                               : cfg_.mrs_[q.common_.mrs_[seg]].adj_size_ -
    ...
    for (auto &q : tx.queues_) {
      ...
      for (int mr_num = 0; mr_num < q.common_.mrs_.size(); mr_num++) {
        ...
        if (mr.num_bufs_ < default_num_tx_desc) {
          DAQIRI_LOG_CRITICAL("Must have at least {} buffers in each TX MR",
                              default_num_tx_desc);
          return;
        }
    ...
    const size_t requested_mtu = (requested_frame_size > crc_and_hdr)
                                     ? (requested_frame_size - crc_and_hdr)
                                     : 0;
    ...
    DAQIRI_LOG_INFO("Setting port config for port {} mtu:{}", intf.port_id_,
                    local_port_conf[intf.port_id_].rxmode.mtu);

    if (single_seg_rx_needs_scatter) {
      if (loopback_ != LoopbackType::LOOPBACK_TYPE_SW) {
        if ((dev_info.rx_offload_capa & RTE_ETH_RX_OFFLOAD_SCATTER) == 0) {
          DAQIRI_LOG_CRITICAL("Single-segment RX queue present (min buffer "
                              "size {}). NIC does not "
        ...
        DAQIRI_LOG_INFO("Enabling RX scatter offload for single-segment RX "
                        "queues (min buffer size: {})",
                        min_single_seg_rx_buf_size);
      } else {
        DAQIRI_LOG_WARN("Single-segment RX queue appears to need scatter in SW "
                        "loopback; skipping offload check");
      }
    }
    ...
    if (loopback_ != LoopbackType::LOOPBACK_TYPE_SW && tx.accurate_send_) {
      ...
      if (ret != 0) {
        ...
        if ((dev_info.tx_offload_capa & RTE_ETH_RX_OFFLOAD_TIMESTAMP) == 0) {
          DAQIRI_LOG_CRITICAL("Accurate send scheduling enabled in config, but "
                              "not supported by NIC!");
          return;
        } else {
    ...
    DAQIRI_LOG_INFO(
        "Initializing port {} with {} RX queues and {} TX queues...",
        intf.port_id_, intf.rx_.queues_.size(), intf.tx_.queues_.size());

    if (loopback_ != LoopbackType::LOOPBACK_TYPE_SW) {
      ret = rte_eth_dev_configure(intf.port_id_, intf.rx_.queues_.size(),
                                  intf.tx_.queues_.size(),
      ...
      if (ret < 0) {
        DAQIRI_LOG_CRITICAL(
            "Cannot adjust number of descriptors: err={}, port={}", ret,
            intf.port_id_);
        return;
      } else {
      ...
      for (const auto &q : rx.queues_) {
        ...
        auto socketid = rte_lcore_to_socket_id(
            strtol(q.common_.cpu_core_.c_str(), nullptr, 10));
        uint32_t key = generate_queue_key(intf.port_id_, q.common_.id_);
        auto qinfo = rx_dpdk_q_map_[key];

        DAQIRI_LOG_INFO("Setting up port:{}, queue:{}, Num scatter:{} pool:{}",
                        intf.port_id_, q.common_.id_, q.common_.mrs_.size(),
                        (void *)qinfo->pools[0]);
        if (q.common_.mrs_.size() > 1) {
          ret = rte_eth_rx_queue_setup(intf.port_id_, q.common_.id_,
                                       default_num_rx_desc, socketid,
                                       &qinfo->rxconf_qsplit, NULL);
        } else {
          ret = rte_eth_rx_queue_setup(intf.port_id_, q.common_.id_,
                                       default_num_rx_desc, socketid, NULL,
                                       qinfo->pools[0]);
        }

        if (ret < 0) {
          DAQIRI_LOG_CRITICAL("rte_eth_rx_queue_setup: err={}, port={}", ret,
                              intf.port_id_);
          return;
        } else {
          DAQIRI_LOG_INFO("Successfully setup RX port {} queue {}",
      ...
      for (const auto &q : tx.queues_) {
        ...
        ret = rte_eth_tx_queue_setup(
            intf.port_id_, q.common_.id_, default_num_tx_desc,
            rte_eth_dev_socket_id(intf.port_id_), &txq_conf);
        if (ret < 0) {
          DAQIRI_LOG_CRITICAL(
              "Queue setup error {}:{}, port={} caps={:x} set={:x}", ret,
              rte_strerror(ret), intf.port_id_, dev_info.tx_offload_capa,
              local_port_conf[intf.port_id_].txmode.offloads);
          return;
        } else {
          DAQIRI_LOG_INFO("Successfully set up TX queue {}/{}", intf.port_id_,
                          q.common_.id_);
        }
      }
      ...
      if (!intf.rx_.flow_isolation_) {
        ...
        DAQIRI_LOG_INFO("Not enabling promiscuous mode on port {} "
      ...
      if (ret != 0) {
        DAQIRI_LOG_CRITICAL("Cannot start device err={}, port={}", ret,
                            intf.port_id_);
        return;
      } else {
      ...
      DAQIRI_LOG_INFO(
          "Port {}, MAC address: {:02X}:{:02X}:{:02X}:{:02X}:{:02X}:{:02X}",
          intf.port_id_, conf_ports_eth_addr[intf.port_id_].addr_bytes[0],
      ...
      for (const auto &flow : rx.flows_) {
        ...
        if (flow.match_.type_ == FlowMatchType::FLEX_ITEM) {
          add_flex_item_flow(intf.port_id_, flow.match_.flex_item_match_,
                             flow.action_.id_);
        } else {
...
int DpdkMgr::setup_pools_and_rings(int max_rx_batch, int max_tx_batch) {
  ...
  for (int i = 0; i < cfg_.ifs_.size(); i++) {
    ...
    for (int j = 0; j < cfg_.ifs_[i].rx_.queues_.size(); j++) {
      ...
      std::string ring_name =
          "RX_RING_P" + std::to_string(port_id) + "_Q" + std::to_string(q_id);
      ...
      if (ring == nullptr) {
        DAQIRI_LOG_CRITICAL("Failed to allocate ring {}! err={}", ring_name,
                            rte_strerror(rte_errno));
        return -1;
      }
  ...
  DAQIRI_LOG_INFO("Setting up RX burst pool with {} batches of size {}",
                  num_rx_ptrs_buffers, sizeof(void *) * max_rx_batch);
  rx_burst_buffer = rte_mempool_create(
      "RX_BURST_POOL", num_rx_ptrs_buffers, sizeof(void *) * max_rx_batch, 0, 0,
  ...
  rx_flow_id_buffer = rte_mempool_create(
      "RX_FLOWID_POOL", num_rx_ptrs_buffers,
      sizeof(ExtraRxPacketInfo) * max_rx_batch, 0, 0, nullptr, nullptr, nullptr,
      nullptr, rte_socket_id(), 0);
  if (rx_flow_id_buffer == nullptr) {
    DAQIRI_LOG_CRITICAL("Failed to allocate RX burst pool!");
    return -1;
  }

  DAQIRI_LOG_INFO("Setting up RX meta pool with {} buffers",
                  cfg_.rx_meta_buffers_);
  rx_metadata = rte_mempool_create("RX_META_POOL", cfg_.rx_meta_buffers_ - 1U,
                                   sizeof(BurstParams), 0, 0, nullptr, nullptr,
  ...
  for (const auto &intf : cfg_.ifs_) {
    for (const auto &q : intf.tx_.queues_) {
      const auto append = "P" + std::to_string(intf.port_id_) + "_Q" +
      ...
      tx_burst_buffers[key] = rte_mempool_create(
          name.c_str(), (1U << 7) - 1U, sizeof(void *) * max_tx_batch, 0, 0,
          nullptr, nullptr, nullptr, nullptr, rte_socket_id(), 0);
  ...
  DAQIRI_LOG_INFO("Setting up TX meta pool with {} buffers",
                  cfg_.tx_meta_buffers_);
  tx_metadata = rte_mempool_create("TX_META_POOL", cfg_.tx_meta_buffers_ - 1U,
                                   sizeof(BurstParams), 0, 0, nullptr, nullptr,
                                   nullptr, nullptr, rte_socket_id(), 0);
  ...
  if (loopback_ == LoopbackType::LOOPBACK_TYPE_SW) {
    loopback_ring = rte_ring_create("LOOPBACK_RING", 4096, rte_socket_id(),
                                    RING_F_MC_RTS_DEQ | RING_F_MP_RTS_ENQ);
...
DpdkMgr::create_flex_flow_rule(int port, int offset,
...

  {
    ...
    int res = rte_flow_validate(port, &jump_attr, jump_pattern, jump_actions,
                                &jump_error);
    if (!res) {
      struct rte_flow *flow = rte_flow_create(port, &jump_attr, jump_pattern,
                                              jump_actions, &jump_error);
      if (flow == NULL) {
  ...
  flex_conf.next_header.field_base =
      32 * 8; // Always sample 8 32-bit words for now
  ...
  sample_data[0].field_base =
      offset * 8; // Offset is in bytes while DPDK wants bits
...
struct rte_flow *DpdkMgr::add_flex_item_flow(int port,
  ...
  int res;
  const auto &flex_item_config =
      cfg_.ifs_[port].rx_.flex_items_[match_info.flex_item_id_];
  ...
  if (flex_item_handles_.find(match_info.flex_item_id_) ==
      flex_item_handles_.end()) {
    struct rte_flow_item_flex_handle *item_handle = create_flex_flow_rule(
        port, flex_item_config.offset_, &udp_item, &pattern[4]);
    flex_item_handles_[match_info.flex_item_id_] = item_handle;
  }

  struct rte_flow_item_flex_handle *item_handle =
      flex_item_handles_[match_info.flex_item_id_];
  ...
  struct rte_flow_item_flex spec = {.handle =
                                        item_handle, /* opaque item handle */
                                    .length = sizeof(struct rte_udp_flex_hdr),
                                    .pattern = (const uint8_t *)&flex_spec};

  struct rte_flow_item_flex mask = {.handle =
                                        item_handle, /* opaque item handle */
                                    .length = sizeof(struct rte_udp_flex_hdr),
                                    .pattern = (const uint8_t *)&flex_mask};

  /* Initialize RTE Flow item itself */
  struct rte_flow_item item = {.type = RTE_FLOW_ITEM_TYPE_FLEX,
...
struct rte_flow *DpdkMgr::add_flow(int port, const FlowConfig &cfg) {
  ...
  // packets
  {
    ...
    auto res = rte_flow_validate(port, &jump_attr, jump_pattern, jump_actions,
                                 &jump_error);
    if (!res) {
      struct rte_flow *flow = rte_flow_create(port, &jump_attr, jump_pattern,
                                              jump_actions, &jump_error);
  ...
  attr.ingress = 1;
  attr.priority =
      1; // Lower priority to allow drop_traffic (priority 0) to take precedence
  attr.group = 3;
...
Status DpdkMgr::drop_all_traffic(int port) {
  ...
  // Initialize the jump rule to group 3 (required by HWS)
  {
    ...
    auto res = rte_flow_validate(port, &jump_attr, jump_pattern, jump_actions,
                                 &jump_error);
    if (!res) {
      config.jump = rte_flow_create(port, &jump_attr, jump_pattern,
                                    jump_actions, &jump_error);
      if (config.jump == nullptr) {
        DAQIRI_LOG_ERROR(
            "rte_flow_create failed for jump rule in drop_all_traffic");
      }
    } else {
      DAQIRI_LOG_ERROR(
          "Failed flow validation for jump rule in drop_all_traffic: {}", res);
    }
  }
  ...
  if (config.drop == nullptr) {
    DAQIRI_LOG_ERROR(
        "Failed to create drop all traffic flow rule on port {}: {}", port,
        error.message ? error.message : "unknown error");
    rte_flow_destroy(port, config.jump, &error);
    return Status::INTERNAL_ERROR;
  } else {
    DAQIRI_LOG_INFO("Successfully created drop all traffic rule on port {}",
                    port);
  }
...
Status DpdkMgr::allow_all_traffic(int port) {
  ...
  int drop_ret =
      rte_flow_destroy(port, drop_all_traffic_flow[port].drop, &drop_error);
  int jump_ret =
      rte_flow_destroy(port, drop_all_traffic_flow[port].jump, &jump_error);

  if (drop_ret != 0) {
    DAQIRI_LOG_ERROR(
        "Failed to destroy drop all traffic flow rule on port {}: {}", port,
        drop_error.message ? drop_error.message : "unknown error");
  }

  if (jump_ret != 0) {
    DAQIRI_LOG_ERROR(
        "Failed to destroy jump all traffic flow rule on port {}: {}", port,
  ...
  DAQIRI_LOG_INFO("Successfully removed drop all traffic rule on port {}, "
                  "traffic is now allowed",
                  port);
  return Status::SUCCESS;
}

struct rte_flow *DpdkMgr::add_modify_flow_set(int port, int queue,
  ...
  for (const auto &q : cfg_.ifs_[port].tx_.queues_) {
    for (const auto &off : q.common_.offloads_) {
      if (off == "tx_eth_src") { // Offload Ethernet source copy
        ...
        add_modify_flow_set(port, q.common_.id_,
                            reinterpret_cast<const char *>(&mac_bytes),
                            sizeof(mac_bytes) * 8, Direction::TX);
      }
    }
  }
}
...
void DpdkMgr::PrintDpdkStats(int port) {
  ...
  if (ret < 0 || ret > len) {
    free(xstats);
    rte_exit(EXIT_FAILURE, "rte_eth_xstats_get(%u) len%i failed: %d", 0, len,
             ret);
  }
  xstats_names =
      (struct rte_eth_xstat_name *)calloc(len, sizeof(*xstats_names));
  ...
  if (ret < 0 || ret > len) {
    free(xstats);
    free(xstats_names);
    rte_exit(EXIT_FAILURE, "rte_eth_xstats_get_names(%u) len%i failed: %d", 0,
             len, ret);
  }
  for (int i = 0; i < len; i++) {
    if (xstats[i].value > 0)
  ...
  for (const auto &intf : cfg_.ifs_) {
    ...
    for (const auto &flow : intf.rx_.flows_) {
      if (flow_to_queue.find(flow.id_) != flow_to_queue.end()) {
        DAQIRI_LOG_ERROR("Duplicate flow ID {} on interface '{}'", flow.id_,
                         intf.name_);
        return false;
      }
    ...
    for (const auto &reorder_cfg : intf.rx_.reorder_configs_) {
      ...
      if (!use_gpu_backend && !use_cpu_backend) {
        DAQIRI_LOG_ERROR(
            "Unsupported reorder_type '{}' for config '{}' on interface '{}'",
            reorder_cfg.reorder_type_, reorder_cfg.name_, intf.name_);
        return false;
      }
      ...
      for (const auto flow_id : reorder_cfg.flow_ids_) {
        ...
        if (flow_queue_it == flow_to_queue.end()) {
          DAQIRI_LOG_ERROR("Reorder config '{}' references unknown flow ID {} "
                           "on interface '{}'",
                           reorder_cfg.name_, flow_id, intf.name_);
          return false;
        }
        ...
        } else if (reorder_queue_id !=
                   static_cast<int>(flow_queue_it->second)) {
          DAQIRI_LOG_ERROR("Reorder config '{}' has flow IDs mapped to "
                           "multiple queues on interface '{}'. "
                           "Each reorder config must map to a single RX queue",
                           reorder_cfg.name_, intf.name_);
          return false;
        }
        if (reorder_flow_ids.find(flow_id) != reorder_flow_ids.end()) {
          DAQIRI_LOG_ERROR("Flow ID {} appears in multiple reorder configs on "
                           "interface '{}'. "
      ...
      if (queue_it == intf.rx_.queues_.end()) {
        DAQIRI_LOG_ERROR(
            "Reorder config '{}' maps to unknown RX queue {} on interface '{}'",
            reorder_cfg.name_, reorder_queue_id, intf.name_);
        return false;
      }
      if (queue_it->common_.mrs_.size() != 1) {
        DAQIRI_LOG_ERROR(
            "Queue '{}'/{} using reorder config '{}' must define exactly one "
            "source memory "
            "region. Header-data split reordering is not supported.",
            queue_it->common_.name_, queue_it->common_.id_, reorder_cfg.name_);
        return false;
      }
      const auto source_mr_it = cfg_.mrs_.find(queue_it->common_.mrs_[0]);
      if (source_mr_it == cfg_.mrs_.end()) {
        DAQIRI_LOG_ERROR("Queue '{}'/{} using reorder config '{}' references "
                         "unknown source MR '{}'",
                         queue_it->common_.name_, queue_it->common_.id_,
                         reorder_cfg.name_, queue_it->common_.mrs_[0]);
        return false;
      }
      if (use_gpu_backend &&
          !is_cuda_accessible_packet_memory(source_mr_it->second.kind_)) {
        DAQIRI_LOG_ERROR("Queue '{}'/{} using reorder config '{}' must use "
                         "DEVICE or HOST_PINNED packet "
      ...
      if (use_cpu_backend &&
          !is_cpu_accessible_memory(source_mr_it->second.kind_)) {
        DAQIRI_LOG_ERROR("Queue '{}'/{} using reorder config '{}' must use "
                         "HOST, HOST_PINNED, or HUGE packet "
                         "memory for CPU reorder",
                         queue_it->common_.name_, queue_it->common_.id_,
                         reorder_cfg.name_);
        return false;
      ...
      if (use_gpu_backend && output_mr_it->second.kind_ != MemoryKind::DEVICE &&
        ...
        DAQIRI_LOG_ERROR("Reorder output memory region '{}' in config '{}' "
                         "must be DEVICE or HOST_PINNED "
                         "memory for GPU reorder",
                         reorder_cfg.memory_region_, reorder_cfg.name_);
        return false;
      }
      if (use_cpu_backend &&
          !is_cpu_accessible_memory(output_mr_it->second.kind_)) {
        DAQIRI_LOG_ERROR("Reorder output memory region '{}' in config '{}' "
                         "must be HOST, HOST_PINNED, or HUGE "
                         "memory for CPU reorder",
                         reorder_cfg.memory_region_, reorder_cfg.name_);
        return false;
      }
      ...
      if (use_data_type_conversion && use_cpu_backend) {
        DAQIRI_LOG_ERROR("Reorder config '{}' data type conversion is "
                         "supported only for gpu reorder_type",
                         reorder_cfg.name_);
        return false;
      }
      ...
      if (source_slot_stride_size >
          static_cast<size_t>(std::numeric_limits<uint32_t>::max())) {
        DAQIRI_LOG_ERROR("Reorder config '{}' source slot size {} is too large",
                         reorder_cfg.name_, source_slot_stride_size);
        return false;
      }
      const auto source_slot_stride =
          static_cast<uint32_t>(source_slot_stride_size);
      ...
      if (!compute_reorder_max_output_payload_len(
        ...
        DAQIRI_LOG_ERROR(
            "Reorder config '{}' cannot convert source slot size {} from {} to "
            "{}",
            reorder_cfg.name_, source_slot_stride,
            reorder_data_type_to_string(reorder_cfg.data_types_.input_type_),
            reorder_data_type_to_string(reorder_cfg.data_types_.output_type_));
        return false;
      }
      ...
      if (reorder_cfg.method_ == ReorderMethod::SEQ_BATCH_NUMBER) {
        ...
        packets_per_batch =
            reorder_cfg.seq_packets_per_batch_.packets_per_batch_;
      }
      const uint64_t min_required_out =
          static_cast<uint64_t>(output_slot_stride) *
          static_cast<uint64_t>(packets_per_batch);
      if (min_required_out > output_mr_it->second.buf_size_) {
        DAQIRI_LOG_ERROR("Reorder output MR '{}' buffer size {} is too small "
                         "for config '{}' "
                         "(required at least {} = packets_per_batch {} * "
                         "output_slot_stride {})",
                         output_mr_it->second.name_,
                         output_mr_it->second.buf_size_, reorder_cfg.name_,
                         min_required_out, packets_per_batch,
                         output_slot_stride);
        return false;
      }
    }
...
void DpdkMgr::run() {
  ...
  for (const auto &el : this->rx_core_q_map) {
    ...
    if (el.second.size() == 1) {
      ...
      if (loopback_ == LoopbackType::LOOPBACK_TYPE_SW) {
        DAQIRI_LOG_CRITICAL(
            "Loopback mode not yet configured for multiple RX queues!");
        exit(1);
      }
      // Multi-q worker
      ...
      for (const auto &q_info : el.second) {
        ...
        params->q_params.push_back({port_id, q_id, (int)q->common_.mrs_.size(),
                                    q->common_.batch_size_, q->timeout_us_,
                                    ring_ptr});
      }
...
int DpdkMgr::rx_core_multi_q_worker(void *arg) {
  ...
  DAQIRI_LOG_INFO("Starting multi-queue RX Core {}, P/Q: {}, socket {}",
                  rte_lcore_id(), pq_str, rte_socket_id());
  ...
  std::array<std::array<rte_mbuf *, DEFAULT_NUM_RX_BURST>,
             Manager::MAX_RX_Q_PER_CORE>
      mbuf_arr{};
  ...
  while (!force_quit.load()) {
    ...
    if (should_flush) {
      ...
      for (int i = 0; i < num_queues; i++) {
        ...
        if (nb_rx[i] > 0) {
          for (int p = cur_pkt_in_batch[i]; p < cur_pkt_in_batch[i] + nb_rx[i];
               p++) {
        ...
        if (bursts[i] != nullptr) {
          ...
          for (int p = 0; p < bursts[i]->hdr.hdr.num_pkts; p++) {
            rte_pktmbuf_free(
                reinterpret_cast<rte_mbuf **>(bursts[i]->pkts[0])[p]);
          }
          ...
          for (int seg = 0; seg < bursts[i]->hdr.hdr.num_segs; seg++) {
            rte_mempool_put(tparams->rx_burst_pool,
                            (void *)bursts[i]->pkts[seg]);
          }
    ...
    if (bursts[cur_idx] == nullptr) { // Allocate a new burst
      if (rte_mempool_get(tparams->rx_meta_pool,
                          reinterpret_cast<void **>(&bursts[cur_idx])) < 0) {
        DAQIRI_LOG_CRITICAL(
            "Running out of RX meta buffers due to high rates. Either increase "
            "your number of metadata buffers (current: {}) with "
            "`rx_meta_buffers` (will "
            "increase memory usage) or increase your `batch_size` for port {} "
            "queue {} (will "
            "increase latency)",
            tparams->rx_meta_pool_size, cur_port, cur_q);
        exit(1);
      }
      ...
      if (rte_mempool_get(tparams->flowid_pool,
                          reinterpret_cast<void **>(
                              &bursts[cur_idx]->pkt_extra_info)) < 0) {
        DAQIRI_LOG_ERROR("Processing function falling behind. No free CPU "
                         "buffers for packets!");
        continue;
      }
    }
    ...
    if (nb_rx[cur_idx] == 0) {
      ...
      nb_rx[cur_idx] = rte_eth_rx_burst(
          cur_port, cur_q, reinterpret_cast<rte_mbuf **>(&mbuf_arr[cur_idx][0]),
    ...
    for (int p = 0; p < to_copy; p++) {
      if (mbuf_arr[cur_idx][cur_pkt_in_batch[cur_idx] + p]->ol_flags &
    ...
    if (cur_segs > 1) { // Extra work when buffers are scattered
      for (int p = 0; p < to_copy; p++) {
        struct rte_mbuf *mbuf =
            mbuf_arr[cur_idx][cur_pkt_in_batch[cur_idx] + p];
        for (int seg = 1; seg < cur_segs; seg++) {
          mbuf = mbuf->next;
          bursts[cur_idx]->pkts[seg][bursts[cur_idx]->hdr.hdr.num_pkts + p] =
              mbuf;
        }
      }
    }
  ...
  for (int i = 0; i < num_queues; i++) {
    ...
    DAQIRI_LOG_INFO(
        "Total packets received by RX core {} (Port/Queue {}/{}): {}",
        rte_lcore_id(), cur_port, cur_q, total_pkts[i]);
  }
...
int DpdkMgr::rx_core_worker(void *arg) {
  ...
  DAQIRI_LOG_INFO("Starting RX Core {}, port {}, queue {}, socket {}",
                  rte_lcore_id(), tparams->port, tparams->queue,
                  rte_socket_id());
  int nb_rx = 0;
  ...
  while (!force_quit.load()) {
    ...
    if (burst == nullptr) { // Allocate a new burst
      ...
      if (rte_mempool_get(tparams->flowid_pool,
                          reinterpret_cast<void **>(&burst->pkt_extra_info)) <
          0) {
        DAQIRI_LOG_ERROR("Processing function falling behind. No free CPU "
                         "buffers for packets!");
        continue;
  ...
  DAQIRI_LOG_INFO(
      "Total packets received by application (port/queue {}/{}): {}",
...
int DpdkMgr::rx_lb_worker(void *arg) {
  ...
  while (!force_quit.load()) {
    ...
    if (rte_mempool_get(tparams->rx_meta_pool,
                        reinterpret_cast<void **>(&meta_burst)) < 0) {
      DAQIRI_LOG_ERROR(
          "Processing function falling behind. No free buffers for metadata!");
      exit(1);
    }
    ...
    for (int seg = 0; seg < tparams->num_segs; seg++) {
      if (rte_mempool_get(tparams->rx_burst_pool,
                          reinterpret_cast<void **>(&meta_burst->pkts[seg])) <
          0) {
        DAQIRI_LOG_ERROR("Processing function falling behind. No free flow ID "
                         "buffers for packets!");
        continue;
      }
    }
    ...
    while (!force_quit.load()) {
      if (rte_ring_dequeue(tparams->lb_ring,
                           reinterpret_cast<void **>(&tx_burst)) == 0) {
...
int DpdkMgr::tx_core_worker(void *arg) {
  ...
  DAQIRI_LOG_INFO("Starting TX Core {}, port {}, queue {} socket {} using "
                  "burst pool {} ring {}",
                  rte_lcore_id(), tparams->port, tparams->queue,
                  rte_socket_id(), (void *)tparams->burst_pool,
                  (void *)tparams->ring);
  ...
  DAQIRI_LOG_INFO(
      ...
      tparams->port, tparams->queue, ttl_pkts_tx);
...
int DpdkMgr::tx_lb_worker(void *arg) {
  ...
  DAQIRI_LOG_INFO("Starting TX Loopback Core {}, port {}, queue {} socket {} "
                  "using burst pool {} ring {}",
                  rte_lcore_id(), tparams->port, tparams->queue,
                  rte_socket_id(), (void *)tparams->burst_pool,
                  (void *)tparams->ring);
  ...
  while (!force_quit.load()) {
    ...
    if (rte_ring_enqueue(tparams->lb_ring, reinterpret_cast<void *>(msg)) !=
        0) {
...
void *DpdkMgr::get_segment_packet_ptr(BurstParams *burst, int seg, int idx) {
  if (burst == nullptr || idx < 0 || seg < 0 || seg >= MAX_NUM_SEGS) {
    return nullptr;
  }
  if ((burst->hdr.hdr.burst_flags & kBurstFlagDpdkReordered) != 0U) {
  ...
  return rte_pktmbuf_mtod(reinterpret_cast<rte_mbuf *>(burst->pkts[seg][idx]),
                          void *);
}

void *DpdkMgr::get_packet_ptr(BurstParams *burst, int idx) {
  if (burst == nullptr || idx < 0) {
    return nullptr;
  }
  if ((burst->hdr.hdr.burst_flags & kBurstFlagDpdkReordered) != 0U) {
  ...
  return rte_pktmbuf_mtod(reinterpret_cast<rte_mbuf *>(burst->pkts[0][idx]),
                          void *);
}

uint32_t DpdkMgr::get_segment_packet_length(BurstParams *burst, int seg,
                                            int idx) {
  if (burst == nullptr || idx < 0 || seg < 0 || seg >= MAX_NUM_SEGS) {
    return 0;
  }
...
Status DpdkMgr::set_packet_tx_time(BurstParams *burst, int idx,
                                   uint64_t timestamp) {
  reinterpret_cast<struct rte_mbuf **>(burst->pkts[0])[idx]->ol_flags |=
      timestamp_mask_;
...
Status DpdkMgr::get_tx_packet_burst(BurstParams *burst) {
  const uint32_t key =
      generate_queue_key(burst->hdr.hdr.port_id, burst->hdr.hdr.q_id);
  const auto q_it = tx_dpdk_q_map_.find(key);
  if (q_it == tx_dpdk_q_map_.end() || q_it->second == nullptr) {
    DAQIRI_LOG_ERROR("Invalid TX queue key {} for port {} queue {}", key,
  ...
  for (int seg = 0; seg < burst->hdr.hdr.num_segs; seg++) {
    if (rte_mempool_get(burst_pool->second,
                        reinterpret_cast<void **>(&burst->pkts[seg])) != 0) {
      return Status::NO_FREE_BURST_BUFFERS;
    }

    if (rte_pktmbuf_alloc_bulk(
            q->pools[seg], reinterpret_cast<rte_mbuf **>(burst->pkts[seg]),
            static_cast<int>(burst->hdr.hdr.num_pkts)) != 0) {
      rte_mempool_put(burst_pool->second,
                      reinterpret_cast<void *>(burst->pkts[seg]));
      return Status::NO_FREE_PACKET_BUFFERS;
    }
  }
...
Status DpdkMgr::set_eth_header(BurstParams *burst, int idx, char *dst_addr) {
  ...
  memcpy(reinterpret_cast<void *>(&mbuf_data->eth.dst_addr),
         reinterpret_cast<void *>(dst_addr), sizeof(mbuf_data->eth.dst_addr));

  mbuf_data->eth.ether_type = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4);
  return Status::SUCCESS;
}

Status DpdkMgr::set_ipv4_header(BurstParams *burst, int idx, int ip_len,
...
Status DpdkMgr::set_udp_header(BurstParams *burst, int idx, int udp_len,
...
Status DpdkMgr::set_udp_payload(BurstParams *burst, int idx, void *data,
                                int len) {
  auto mbuf = reinterpret_cast<rte_mbuf *>(burst->pkts[0][idx]);
  auto mbuf_data = rte_pktmbuf_mtod(mbuf, UDPPkt *);
...
Status DpdkMgr::set_packet_lengths(BurstParams *burst, int idx,
  ...
  uint32_t ttl_len = 0;
  for (int seg = 0; seg < burst->hdr.hdr.num_segs; seg++) {
    reinterpret_cast<rte_mbuf **>(burst->pkts[seg])[idx]->data_len =
        *(lens.begin() + seg);
...
Status DpdkMgr::set_all_packet_lengths(BurstParams *burst,
  ...
  for (int seg = 0; seg < burst->hdr.hdr.num_segs; ++seg) {
    ...
    for (int idx = 0; idx < num_pkts; ++idx) {
      mbufs[idx]->data_len = seg_lens[seg];
    }
  }

  auto **first_seg = reinterpret_cast<rte_mbuf **>(burst->pkts[0]);
  for (int idx = 0; idx < num_pkts; ++idx) {
    first_seg[idx]->pkt_len = ttl_len;
  }
...
void DpdkMgr::free_packet(BurstParams *burst, int pkt) {
  ...
  if ((burst->hdr.hdr.burst_flags & kBurstFlagDpdkReordered) != 0U) {
    release_reorder_output_context(burst);
    if (burst->pkt_lens[0] != nullptr && pkt >= 0 &&
        pkt < static_cast<int>(burst->hdr.hdr.num_pkts)) {
      burst->pkt_lens[0][pkt] = 0;
    }
    return;
...
Status DpdkMgr::get_rx_burst(BurstParams **burst, int port, int q) {
  ...
  const auto ring_it = rx_rings.find(key);
  if (ring_it == rx_rings.end()) {
    DAQIRI_LOG_ERROR("Invalid port/queue combination in get_rx_burst: {}/{}",
                     port, q);
    return Status::INVALID_PARAMETER;
  }
...
Status DpdkMgr::get_tx_metadata_buffer(BurstParams **burst) {
  if (rte_mempool_get(tx_metadata, reinterpret_cast<void **>(burst)) != 0) {
    DAQIRI_LOG_CRITICAL(
        ...
        "queue {} (will increase "
        "latency)",
        cfg_.tx_meta_buffers_, (*burst)->hdr.hdr.port_id,
        (*burst)->hdr.hdr.q_id);
    return Status::NO_FREE_BURST_BUFFERS;
  }
...
void DpdkMgr::print_stats() {
  ...
  int portid;
  RTE_ETH_FOREACH_DEV(portid) { PrintDpdkStats(portid); }
}
