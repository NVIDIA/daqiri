...
struct ExternalBatchProducerOptions {
  ...
  int cpu_core{-1};
  MemoryKind memory_kind{MemoryKind::host_pinned_mapped};
  // The UCX-only benchmark waits for one receiver batch-slot credit so it
...
struct TransportStats {
  ...
  std::uint64_t batches_released{0};
...
class ReceivedBatch {
 public:
  ...
  ReceivedBatch(const ReceivedBatch&) = delete;
  ReceivedBatch& operator=(const ReceivedBatch&) = delete;
  ReceivedBatch(ReceivedBatch&& other) noexcept;
  ReceivedBatch& operator=(ReceivedBatch&& other) = delete;
...
enum class ReceiveStatus { batch, end_of_stream, timeout, failed };

struct ReceiveResult {
  ReceiveStatus status{ReceiveStatus::timeout};
  std::optional<ReceivedBatch> batch;
  std::string error;
};
...
// - exactly one RX/orchestration thread calls try_acquire();
// - exactly one processing thread calls submit_after();
//   that same thread calls discard() for rejected or unused input batches;
...
class Receiver {
 public:
  ...
  // The thread that calls start() owns the single-threaded UCP worker. That
  // same thread must call receive(), release()/release_after(), and close().
  void start();
  ReceiveResult receive(std::chrono::milliseconds timeout);
  void release(ReceivedBatch batch);
  void release_after(ReceivedBatch batch, cudaStream_t stream);
