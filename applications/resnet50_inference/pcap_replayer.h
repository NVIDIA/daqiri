...
// linktype 1 / Ethernet). Loads frames into memory for replay, capped by
// per-record size plus cumulative frame-count and total-byte limits (see
// pcap_replayer.cpp) so a large CLI-selected pcap cannot exhaust host RAM.
class PcapReplayer {
 public:
