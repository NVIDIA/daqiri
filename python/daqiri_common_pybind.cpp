...
PYBIND11_MODULE(_daqiri, m) {
  ...
  m.def(
        ...
        try {
          ...
          if (py::hasattr(config_obj, "value")) {
            ...
            try {
              ...
              DAQIRI_LOG_ERROR("Failed to import yaml module or convert dict to YAML: {}",
                                 e.what());
          ...
          if (status == Status::SUCCESS) {
            DAQIRI_LOG_INFO("Successfully initialized DAQIRI from Python config");
          }
          return status;
      ...
      "config"_a,
      "Initialize the DAQIRI backend from config object");
  ...
  m.def("print_stats", &print_stats, "Print statistics from the daqiri manager");
