...
PYBIND11_MODULE(_daqiri, m) {
  ...
  m.def(
      "get_rx_burst_managed",
      [](int port, int q) {
        RxBurst owner;
        Status status = Status::SUCCESS;
        {
          py::gil_scoped_release release;
          status = get_rx_burst(&owner, port, q);
        }
        return py::make_tuple(status, std::move(owner));
      },
      "port"_a, "q"_a);
  m.def(
      "get_rx_burst_managed",
      [](int port) {
        ...
        {
          py::gil_scoped_release release;
          status = get_rx_burst(&owner, port);
        }
        return py::make_tuple(status, std::move(owner));
      },
      "port"_a);
  m.def("get_rx_burst_managed", []() {
    RxBurst owner;
    Status status = Status::SUCCESS;
    {
      py::gil_scoped_release release;
      status = get_rx_burst(&owner);
    }
    return py::make_tuple(status, std::move(owner));
  });
  m.def(
      "get_rx_burst_for_connection_managed",
      [](uintptr_t conn_id, bool server) {
        RxBurst owner;
        Status status = Status::SUCCESS;
        {
          py::gil_scoped_release release;
          status = get_rx_burst(&owner, conn_id, server);
        }
        return py::make_tuple(status, std::move(owner));
      },
      "conn_id"_a, "server"_a);
  m.def("create_tx_burst_managed", []() {
    TxBurst owner;
    const Status status = create_tx_burst(&owner);
    return py::make_tuple(status, std::move(owner));
  });
