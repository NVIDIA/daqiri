...
PYBIND11_MODULE(_daqiri, m) {
  m.doc() = "DAQIRI utility functions";
  ...
  m.def(
        ...
        try {
          ...
          if (py::hasattr(config_obj, "value")) {
            ...
            try {
              ...
              DAQIRI_LOG_ERROR(
                  "Failed to import yaml module or convert dict to YAML: {}",
                  e.what());
              DAQIRI_LOG_ERROR("Please install PyYAML: pip install PyYAML");
          ...
          DAQIRI_LOG_DEBUG("Attempting to parse YAML config: {}", yaml_str);
          ...
          auto status = daqiri_init(config);
          if (status == Status::SUCCESS) {
            DAQIRI_LOG_INFO(
                "Successfully initialized DAQIRI from Python config");
          }
          return status;
      ...
      "config"_a, "Initialize the DAQIRI backend from config object");
  ...
  m.def("get_segment_packet_length",
        ...
        "burst"_a, "seg"_a, "idx"_a, "Get length of one segment of the packet");
  m.def("get_packet_length",
        py::overload_cast<BurstParams *, int>(&get_packet_length), "burst"_a,
        "idx"_a, "Get length of the packet");
  m.def("get_packet_flow_id",
        py::overload_cast<BurstParams *, int>(&get_packet_flow_id), "burst"_a,
        "idx"_a, "Get flow ID of a packet");

  // TX burst functions
  m.def("is_tx_burst_available",
        py::overload_cast<BurstParams *>(&is_tx_burst_available), "burst"_a,
  ...
  m.def("get_tx_packet_burst",
        ...
        "Get TX packet burst");
  m.def("send_tx_burst", py::overload_cast<BurstParams *>(&send_tx_burst),
        "burst"_a, "Send a TX burst");
  ...
  m.def(
      ...
      [](int port, int q) {
        ...
        return py::make_tuple(
            status, py::cast(burst_ptr, py::return_value_policy::reference));
      },
      "port"_a, "q"_a,
  ...
  m.def("set_ipv4_header",
        py::overload_cast<BurstParams *, int, int, uint8_t, unsigned int,
                          unsigned int>(&set_ipv4_header),
        "burst"_a, "idx"_a, "ip_len"_a, "proto"_a, "src_host"_a, "dst_host"_a,
        "Set IPv4 header in packet");
  m.def("set_udp_header",
        py::overload_cast<BurstParams *, int, int, uint16_t, uint16_t>(
            &set_udp_header),
        "burst"_a, "idx"_a, "udp_len"_a, "src_port"_a, "dst_port"_a,
  ...
  m.def("get_num_packets", py::overload_cast<BurstParams *>(&get_num_packets),
        "burst"_a, "Get number of packets in a burst");
  m.def("get_q_id", py::overload_cast<BurstParams *>(&get_q_id), "burst"_a,
        "Get queue ID of a burst");
  m.def("set_num_packets",
        py::overload_cast<BurstParams *, int64_t>(&set_num_packets), "burst"_a,
        "num"_a, "Set number of packets in a burst");
  m.def("set_header",
        py::overload_cast<BurstParams *, uint16_t, uint16_t, int64_t, int>(
            &set_header),
        "burst"_a, "port"_a, "q"_a, "num"_a, "segs"_a,
  ...
  m.def("free_all_packets_and_burst_rx",
        ...
        "burst"_a, "Free all packets and burst structure for RX");
  m.def("free_all_packets_and_burst_tx",
        py::overload_cast<BurstParams *>(&free_all_packets_and_burst_tx),
        "burst"_a, "Free all packets and burst structure for TX");
  ...
  m.def("free_tx_burst", py::overload_cast<BurstParams *>(&free_tx_burst),
        "burst"_a, "Free TX burst");
  m.def("free_rx_burst", py::overload_cast<BurstParams *>(&free_rx_burst),
        "burst"_a, "Free RX burst");
  ...
  m.def("get_port_id", py::overload_cast<const std::string &>(&get_port_id),
        "key"_a, "Get port number from interface name");

  // Utility functions
  m.def("shutdown", &shutdown, "Shut down the daqiri manager");
  m.def("print_stats", &print_stats,
        "Print statistics from the daqiri manager");
