...
- Removed the `daqiri_tx` (`AdvNetworkOpTx`) and `daqiri_rx` operators. See <https://github.com/nvidia-holoscan/holohub/pull/743>. Instead:
  - Initialize your NIC with `daqiri::daqiri_init` in your application.
  - In your custom Tx operator, replace `op_output.emit` with `daqiri::send_tx_burst`.
  - In your custom Rx operator, replace `op_input.receive` with `daqiri::get_rx_burst`, also specifying the port and queue ids.
...
- **Enhanced Network Benchmarking:**
  ...
  - Added a new configuration (`daqiri_bench_default_tx_rx_hds.yaml`) for benchmarking multi-queue transmit/receive scenarios with Header-Data Split.
...
- `daqiri::free_all_packets_and_burst_rx` and `daqiri::free_all_packets_and_burst_tx` replace legacy free helpers. See <https://github.com/nvidia-holoscan/holohub/pull/735>.
