...
OUT_ROOT="${DAQIRI_BENCH_RESULTS_DIR:-$REPO_ROOT/bench-results}"
if ! mkdir -p "$OUT_ROOT" 2>/dev/null || [[ ! -w "$OUT_ROOT" ]]; then
  if [[ -n "${DAQIRI_BENCH_RESULTS_DIR:-}" ]]; then
    echo "ERROR: cannot write DAQIRI_BENCH_RESULTS_DIR=$OUT_ROOT" >&2
    exit 1
  fi
  OUT_ROOT="/tmp/daqiri-bench-results"
  mkdir -p "$OUT_ROOT"
fi
OUT_DIR="$OUT_ROOT/${TS}-rtx-pro-${BACKEND}-${MODE}"
mkdir -p "$OUT_DIR"
...
case "$BACKEND" in
  dpdk)
    ...
    ;;
  dpdk-hds)
    PAYLOADS_SWEEP=(8000 4096 2048)
    BATCHES_SWEEP=(4096 2048 1024)
    PAYLOADS_HEADLINE=(8000)
    BATCHES_HEADLINE=(4096)
