...
ReorderPipeline::~ReorderPipeline() { destroy(); }
...
bool ReorderPipeline::init(ReorderMode mode, uint32_t packets_per_batch,
                           uint32_t out_payload_len, uint32_t payload_byte_offset,
                           uint16_t seq_bit_offset, uint8_t seq_bit_width,
  ...
  if (cudaMalloc(&dev_ptrs_, static_cast<size_t>(packets_per_batch) *
                                 sizeof(const void *)) != cudaSuccess) {
  ...
  if (staging_needed_) {
    if (cudaMalloc(&staging_, static_cast<size_t>(packets_per_batch) *
                                  stage_slot_bytes_) != cudaSuccess) {
...
void *ReorderPipeline::stage_host_packet(const void *hptr, uint32_t len) {
  if (!ok_ || !staging_needed_ || hptr == nullptr ||
      staged_ >= packets_per_batch_) {
    return nullptr;
  }
  const uint32_t copy_len =
      len < stage_slot_bytes_ ? len : static_cast<uint32_t>(stage_slot_bytes_);
  auto *dst = static_cast<uint8_t *>(staging_) +
              static_cast<size_t>(staged_) * stage_slot_bytes_;
  cudaMemcpyAsync(dst, hptr, copy_len, cudaMemcpyHostToDevice,
                  as_stream(stream_));
  ++staged_;
  return dst;
}
...
const void *ReorderPipeline::finish_batch() {
  ...
  if (mode_ == ReorderMode::GatherOnly && collected_ == 1) {
    const auto *src = static_cast<const uint8_t *>(
        static_cast<const void **>(host_ptrs_)[0]);
    return src + payload_byte_offset_;
  }

  cudaMemcpyAsync(dev_ptrs_, host_ptrs_,
                  static_cast<size_t>(collected_) * sizeof(const void *),
  ...
  if (mode_ == ReorderMode::SeqReorder) {
    packet_reorder_copy_payload_by_sequence(
        ordered_, in, out_payload_len_, out_payload_len_, payload_byte_offset_,
        collected_, seq_bit_offset_, seq_bit_width_, /*batch_bit_offset=*/0,
        /*batch_bit_width=*/0, /*has_batch_number=*/0, packets_per_batch_,
        /*max_slot_idx=*/packets_per_batch_ - 1, kDataTypeSame, kDataTypeSame,
        kEndianNetwork, /*batch_id_out=*/nullptr, as_stream(stream_));
  } else {
    packet_gather_copy_payload(ordered_, in, out_payload_len_,
                               payload_byte_offset_, collected_,
                               as_stream(stream_));
  }
  return ordered_;
}
