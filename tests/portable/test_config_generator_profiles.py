...
def test_invalid_profile_inputs_fail_before_rendering() -> None:
    ...
    with pytest.raises(ConfigError, match="runtime frame-size limit"):
        raw_spec(header_size=42, payload_size=9059)
    with pytest.raises(ConfigError, match="runtime frame-size limit"):
        raw_spec(transform="vxlan", header_size=42, payload_size=9009)
    with pytest.raises(ConfigError, match=r"at least header_size \+ payload_size"):
        raw_spec(buffer_size=8000)
...
def test_production_profiles_do_not_require_benchmark_worker_cores() -> None:
    ...
    raw_config = raw["daqiri"]["cfg"]
    assert {region["buf_size"] for region in raw_config["memory_regions"]} == {2048}
    assert "offloads" not in raw_config["interfaces"][0]["tx"]["queues"][0]
